...
import preload_models as model
...
def preload_timing_action():
    ...
    if len(all_data_devices)==0:
        ...
        params = urllib.urlencode({"username": utf_username, "compressed": False,"tasks": json.dumps(all_data_urls), "nest_track_level": 0, "startTime": utf_preload_timing, "validationType": utf_preload_validation, "speed": "","devices": json.dumps(all_data_devices)})
...
def find_devs(dev_id):
    ...
    devs = db_devs.pop("devices").values()
    devs.sort(reverse=True, key=lambda  x:x.get('code'))
    devs.sort(reverse=True, key=lambda  x:x.get('firstLayer'))
    return render_template('preload_query_dev.html', devs=devs,unprocess=db_devs.get("unprocess"),count=len(devs))
