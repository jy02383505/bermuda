...
from models import find_fc_cert_tasks,modify_cert_fc,find_cert_encrypt,modify_cert_encry


LOG_FILENAME = '/Application/bermuda/logs/new_admin.log'
...
def auth():
    try:
        ...
        try:
            ...
            urllib2.ssl._DEFAULT_CIPHERS += 'HIGH:!DH:!aNULL'
            response = urllib2.urlopen('https://sso.chinacache.com/queryByTokenId?clientName=' + config.get('admin', 'client') + '&tokenId=%s&md5Hash=%s' % (ioss, md5.hexdigest()))
        except Exception, e:
            urllib.ssl._DEFAULT_CIPHERS += 'HIGH:!DH:!aNULL'
            response = urllib.urlopen('https://sso.chinacache.com/queryByTokenId?clientName=' + config.get('admin', 'client') + '&tokenId=%s&md5Hash=%s' % (ioss, md5.hexdigest()))
...
@app.route('/cert_only_domain', methods=['GET', 'POST'])
def cert_only_domain():
    totalpage = 0
    args = {"totalpage": totalpage,
            "curpage": int(request.form.get("curpage", 0)),
            "username": request.form.get("username", ""),
            "Is_encry": request.form.get("Is_encry", ""),
            "domain": request.form.get("domain", ""),
            }
    tasks_dict = find_cert_encrypt(args)
    args["totalpage"] = tasks_dict.get("totalpage")
    return render_template('cert_only_domain.html', tasks=tasks_dict.get("tasks"), args=args)


@app.route('/cert_encrypt_send', methods=['GET', 'POST'])
def cert_encrypt_send():
    trigger = session.get('user').account
    app.logger.info('----user trigger %s' % (trigger))
    if not trigger:
        raise
    cert_data = json.loads(request.data)
    app.logger.debug(cert_data)
    username = cert_data.get("username", "")
    cert_id = cert_data.get("cert_id", "")
    Is_encry = cert_data.get("Is_encry", True)
    cert_query = {"username":username,"cert_id":cert_id,"trigger":trigger,"Is_encry":Is_encry}
    task = {}
    try:
        task = modify_cert_encry(cert_query)
    except Exception,e:
        app.logger.debug(traceback.format_exc(e))
        pass

    return json.dumps(task)

@app.route('/change_cert_fc_status', methods=['GET', 'POST'])
def change_cert_fc_status():
    trigger = session.get('user').account
    app.logger.info('----user trigger %s' % (trigger))
    if not trigger:
        raise
    ...
    username = cert_data.get("username", "")
    cert_id = cert_data.get("cert_id", "")
    cert_query = {"username":username,"cert_id":cert_id,"trigger":trigger}
    task = {}
    try:
        task = modify_cert_fc(cert_query)
    except Exception,e:
        app.logger.debug(traceback.format_exc(e))
        pass

    return json.dumps(task)
@app.route('/cert_fc_send', methods=['GET', 'POST'])
def cert_fc_send():
    totalpage = 0
    ...
    tasks_dict = find_fc_cert_tasks(args)
    args["totalpage"] = tasks_dict.get("totalpage")
    return render_template('cert_fc_send.html', tasks=tasks_dict.get("tasks"), args=args)
...
def query_url():
    ...
    "query_type": request.form.get("query_type", 'normal_query'),
    "start_datetime":request.form.get("start_datetime",(datetime.now() - timedelta(hours=1)).strftime('%Y-%m-%d %H:%M')),
    "end_datetime":request.form.get("end_datetime",(datetime.now() + timedelta(hours=1)).strftime('%Y-%m-%d %H:%M')),
    "channel_name":request.form.get("channel_name", "")
    }
    urls_dict = find_urls(args,True)
...
def user_retry_url():
    ...
    try:
        ...
        app.logger.error('retry_api error:%s' % traceback.format_exc(e))
...
def change_push_by_rcms():
    ...
    data = dict((key, request_args.getlist(key) if len(request_args.getlist(key)) > 1 else request_args.getlist(key)[0]) for key in request_args.keys())
    try:
