...
from core.cert_trans_worker import do_callback_portal
...
def find_urls(query_args,sencend=False):
    per_page = 30
    ...
    if query_type == 'normal_query':
        ...
        if sencend:
            start_datetime = datetime.strptime(query_args.get("start_datetime"), "%Y-%m-%d %H:%M")
            end_datetime = datetime.strptime(query_args.get("end_datetime"), "%Y-%m-%d %H:%M")
        else:
    ...
    return urls_dict
def modify_cert_encry(query_map):
    logger.debug(query_map)
    username = query_map.get("username")
    trigger = query_map.get("trigger")
    cert_id = query_map.get("cert_id")
    Is_encry = query_map.get("Is_encry")
    if type(Is_encry) is str:
        Is_encry = True if Is_encry.lower() == "true" else False
    result = {"status": False}
    try:

        monitor_db.cert_only_domain.find_one_and_update({'cert_id': cert_id, "username": username},
                                      {'$set': {'Is_encry': True if (not Is_encry) else False,'Is_change': Is_encry,'trigger': trigger}})
        result = {"status": True}
    except Exception,e:
        logger.debug(traceback.format_exc(e))
        result = {"status": False, "message": "Exception"}
        #result = {"status":False}
    return result

def modify_cert_fc(query_map):
    logger.debug(query_map)
    username = query_map.get("username")
    trigger = query_map.get("trigger")
    cert_id = ObjectId(query_map.get("cert_id"))
    result = {"status":False}
    try:

        portal_success = do_callback_portal(PORTAL_URL, 'portal', {'cert_ids': [query_map.get("cert_id")],'status':'ENABLE'})#需要修改
        if portal_success:
            monitor_db.cert_detail.update({'_id': cert_id, "username": username}, {'$set': {'fc_send': True,'trigger':trigger}})
            monitor_db.cert_trans_tasks.find_one_and_update({'c_id': query_map.get("cert_id"), 'portal_callback_time': ""},{"$set": {'portal_callback_time': datetime.now()}})
            result = {"status": True}
        else:
            result = {"status": False, "message": "portal error"}
    except Exception,e:
        logger.debug(traceback.format_exc(e))
        #result = {"status":False}
    return result

def find_cert_encrypt(query_args):
    '''
      查询边缘加密证书信息
      '''
    ...
    query = {}
    if query_args['domain']:
        query['domain'] = query_args['domain']
    if query_args.get("username"):
        query['username'] = query_args['username']
    if query_args.get("Is_encry"):
        if query_args.get("Is_encry") =="ALL":
            pass
        else:
            if type(query_args.get("Is_encry")) is unicode:
                query['Is_encry'] = True if query_args['Is_encry'].lower() == 'true' else False
            else:
                query['Is_encry'] = query_args['Is_encry']

    if query_args.get("domain") == '' and query_args.get("username") == '' and query_args.get("status") == '':
        logger.debug(query)
        logger.debug("-----------------------------")
        pass
    else:
        query['isTrans'] = False
        logger.debug(query)
        totalpage = int(math.ceil(monitor_db.cert_only_domain.find(query).limit(600).count(True) / (per_page * 1.00)))
        tasks_dict = {"tasks": f_object_str([u for u in monitor_db.cert_only_domain.find(query).skip(query_args.get("curpage") * per_page).limit(per_page)]),
                      "totalpage": totalpage}
    logger.debug(tasks_dict)

    return tasks_dict

def find_fc_cert_tasks(query_args):
    '''
    查询有FC设备的证书信息
    '''
    logger.debug(query_args)
    ...
    #     query['status'] = query_args.get("status")
    #     logger.debug("query:%s" % str(query))
    filter_data = {"username":1,'created_time':1,"cert_alias":1,"fc_send":1}
    if query_args.get("username") == '' and query_args.get("task_id") == '':
        logger.debug(query,filter_data)
        logger.debug("-----------------------------")
        pass
    else:
        query['hasFc'] = True
        logger.debug(query,filter_data)
        totalpage = int(math.ceil(monitor_db.cert_detail.find(query,filter_data).limit(600).count(True)/(per_page*1.00)))
        tasks_dict ={"tasks": f_object_str([u for u in monitor_db.cert_detail.find(query,filter_data).sort('created_time', pymongo.DESCENDING).skip(query_args.get("curpage")*per_page).limit(per_page)] ),"totalpage" : totalpage}
    logger.debug(tasks_dict)
...
def get_devs_cert_task(dev_id):
    ...
    success_strs = []
    failed_devs = []
    for dev in all_devs['devices'].values():
        if dev['code'] != 503:
            res_code = dev['code']
        else:
            res_code = dev['r_code']
...
def get_devs_cert_query_task(dev_id):
    ...
    success_strs = []
    failed_devs = []
    for dev in all_devs['query_dev_ip'].values():
        if dev['code'] != 503:
            res_code = dev['code']
        else:
            res_code = dev['r_code']
...
def get_retry_dev_bran_by_id(retry_branch_id):
    ...
    if not devices_result:
        ...
        for edge_host,v in edge_sub_dic.items():
...
def get_timer_tasks():
    ...
    try:
        for timer in q_db.timer_task.find():
            for key in timer.get("tasks").keys():
...
def retry_api(dev_id):
    ...
    try:
        ...
        logger.error('retry_api error:%s' % traceback.format_exc(e))
    return '服务器连接异常!!'
...
def get_device_monitor_page(page=0, date_str_from=None ,date_str_to=None):
    ...
    all_failed_list = sorted(all_failed.items(), key=lambda x:x[1], reverse=True)
    per_page = 30
...
def get_device_failed_detail_page(page=0, search_start_date=None, search_end_date=None, hostname=None, channel_name=None):
    ...
    # total_page = int(math.ceil(multi_db.device_urls_day.find(query).count(True)/(per_page*1.00)))
    # all_detail = [u for u in multi_db.device_urls_day.find(query).sort('datetime', pymongo.DESCENDING).skip(page*per_page).limit(per_page)]

    total_page = int(math.ceil(multi_db[db_name].find(query).count(True)/(per_page*1.00)))
    all_detail = [u for u in multi_db[db_name].find(query).sort('datetime', pymongo.DESCENDING).skip(page*per_page).limit(per_page)]
...
def del_email_management(_id, user_email):
    ...
    try:
        ...
        logger.debug("del_email_managemet error:%s" % traceback.format_exc(e))
...
def get_province_control_table_reverse():
    ...
    return {value.decode(): key for key, value in data.iteritems()}
...
def operation_log_list(args, query):
    ...
    try:
        ...
        logger.debug('operation_log_list error:%s' % traceback.format_exc(e))
...
def model_update_callback_email(data):
    ...
    if username:
        ...
        try:
            ...
            logger.debug("update_callback_email, update redis error, username:%s, Exception:%s" % (username, traceback.format_exc(e)))
            return json.dumps({'msg': 'error', 'result': 'update_rewrite_no_id  exception:%s' % traceback.format_exc(e)})
...
def model_delete_callback_email(data):
    ...
    try:
        ...
        logger.debug("model_delete_callback_email error, delete data error, id:%s, exception:%s" % (id, traceback.format_exc(e)))
        return json.dumsp({"msg": 'error', 'result': 'del data from mongo error, exception:%s' % traceback.format_exc(e)})
    try:
        CALLBACK_CACHE.delete(prefix_callback_email_username + username)
    except Exception, e:
        logger.debug("model_delete_callback_email error, delete redis error, id:%s, exception:%s" % (id, traceback.format_exc(e)))
        return json.dumps({"msg": 'error', 'result': 'del data from redis error, exception:%s' % traceback.format_exc(e)})
