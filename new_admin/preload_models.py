...
def get_channels(args):
    ...
    channels = [get_device_type(channel) for channel in db.preload_channel.find(
        args).skip(curpage * per_page).limit(per_page)]
...
def get_channels_unopened(username, usertype):
    ...
    if int(usertype) == 1:
        ...
        opened_channels = [c.get("channel_name")
                           for c in db.preload_channel.find({"username": username})]
...
def add_channel(channels, device_type=500):
    ...
    for channel in channels:
        ...
        channel_db = db.preload_channel.find_one(
            {"channel_name": channel.get('channel_name'), "username": channel.get('username')})
        if not channel_db:
...
def config_channel(args):
    ...
    device_type_ori = 3
    if channel.has_key('config_count'):
    ...
    if channel['avoid_period_s'] and channel['avoid_period_e']:
        ...
        channel['period_length'] = (
            avoid_extra_minutes * 60) if (avoid_extra_minutes * 60) > delta_t.seconds else delta_t.seconds
        channel['status'] = 'AVOID'
    else:
        channel['status'] = ''
    channel['first_layer_not_first'] = args.get('first_layer_not_first')
    channel['productType'] = args.get('productType')
...
#        args.pop("username")
    ...
    if not args.get("name"):
        args.pop("name")
    devs = [dev for dev in db.preload_config_device.find(
        args).skip(curpage * per_page).limit(per_page)]
...
def get_devices_all(c_name):
    ...
    channel['devs_info'] = [{'name': dev.get('name'), 'host': dev.get('host'), 'channel_code': dev.get(
        'channel_code', '0'), 'username': dev.get('username')} for dev in db.preload_config_device.find({'channel_name': c_name})]
    dev_list_str = ''
...
def set_devices(channel, dev_list):
    ...
    logger.debug("set_devices dev_list_new: %s|| len(devs_from_cms): %s" %
                 (dev_list_new, len(devs_from_cms)))
    devices_new = [add_dev_fields(d, username=channel.get('username'), channel_code=channel.get(
        'channel_code'), channel_name=channel.get('channel_name')) for d in dev_list_new]
...
def get_devs_unopened(args):
    ...
    rcms_devs = [dev for dev in rcmsapi.getFirstLayerDevices(args.get(
        "channel_code")) + rcmsapi.getDevices(args.get("channel_code")) if dev.get("status") == "OPEN"]
    if dev_name:
...
def del_all_devs(channel_name, username):
    ...
    try:
        ...
        logger.debug('del_all_devs  delete channel_name:%s, and username:%s succes' %
                     (channel_name, username))
    except Exception, e:
        logger.debug('del_all_devs delete channel_name:%s, username:%s error:%s' %
                     (channel_name, username, e))
...
def get_urls(query_args):
    ...
    if query_args.get("status") == "ALL" and query_args.get("username") == '' and query_args.get("url") == '':
        ...
        totalpage = int(math.ceil(s1_db.preload_url.find(
            query).limit(600).count(True) / (per_page * 1.00)))
        urls_dict = {"urls": load_url([u for u in s1_db.preload_url.find(query, {"get_url_speed": 1, "_id": 1, "username": 1, 'url': 1, 'created_time': 1, 'status': 1, 'finish_time': 1, 'compressed': 1, 'dev_id': 1, 'retry_branch_id': 1, 'task_type': 1}).sort('created_time', pymongo.DESCENDING).skip(query_args.get("curpage") * per_page).limit(per_page)]), "totalpage": totalpage}
    return urls_dict
...
def get_result_by_id_new(url_id):
    try:
        ...
        if result and dev_cache:
            ...
            devices = {k: json.loads(v) for k, v in dev_cache.items()}
            res['devices'] = devices
            return res
        else:
            return s1_db.preload_result.find_one({"_id": ObjectId(url_id)})
    except Exception, e:
        logger.debug("get_result_by_id_new[error]: %s" % (traceback.format_exc(e), ))
        return {}
...
def get_queues(query_args):
    ...
    logger.debug("query:%s" % str(query))
    totalpage = int(math.ceil(s1_db.preload_queue_ratio.find(
        query).limit(600).count(True) / (per_page * 1.00)))
    queues_dict = {"queues": [
        queue for queue in s1_db.preload_queue_ratio.find()], "totalpage": totalpage}
    return queues_dict
