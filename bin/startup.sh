...
start_rep() {
    echo "rep is running ..."
    nohup ./bin/gunicorn receiverd_g:app -c /Application/bermuda/conf/gunicorn_config.py > /dev/null 2>&1 &
}
...
start_admin() {
    ...
    nohup ./bin/new_admind > /dev/null 2>&1 &
}
...
start_duowan_monitord() {
    ...
    nohup ./bin/duowan_monitord > /dev/null 2>&1 &
}

start_monitord() {
    echo "monitord is running ..."
    nohup ./bin/monitord > /dev/null 2>&1 &
}

stop_monitord() {
	echo "`date  '+%Y-%m-%d %T'` killing monitord"
	pid=`ps -eo pid,cmd|grep monitord|awk '{print $1}'`
	echo kill $pid
	kill $pid
}
...
stop_duowan_monitord() {
	...
	pid=`ps -eo pid,cmd|grep duowan_monitord|awk '{print $1}'`
	echo kill $pid
	kill $pid
}
...
start_retry_beated() {
    ...
    nohup ./bin/retry_beated > /dev/null 2>&1 &
}

stop_retry_beated() {
	echo "`date  '+%Y-%m-%d %T'` killing retry_beated"
	pid=`ps -eo pid,cmd|grep retry_beated|awk '{print $1}'`
	echo kill $pid
	kill $pid
}
...
start_celery_monitord() {
    ...
    nohup ./bin/celery_monitord > /dev/null 2>&1 &
}

stop_celery_monitord() {
	echo "`date  '+%Y-%m-%d %T'` killing celery monitord"
	pid=`ps -eo pid,cmd|grep celery_monitord|awk '{print $1}'`
	echo kill $pid
	kill $pid
}
