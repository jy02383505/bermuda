...
def upload_html(*args):
    for filename in args:
        put(filename, "/Application/bermuda/static/html")
...
def install_bermuda():
    with cd('/Application/bermuda-4.5.4.3.dev-r68635/'):
        run('/Application/bermuda/bin/python setup.py install')


def restart_fastcgi():
    with cd('/Application/bermuda'):
        run('./restart.sh')


def tail_log(*args):
    env.warn_only = True
    with cd('/Application/bermuda/logs'):
        for log in args:
            run('tail %s' % log)
