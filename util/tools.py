...
def get_channelname(url):
    ...
    try:
       ...
       channelname = u_s[0] + '//' + u_s[2]
        ...
        logger.error('get_channelname error is %s' % (traceback.format_exc(e), ))
    return channelname
...
def strip_list(list_substance):
    ...
    """
    if list_substance:
        for i in xrange(len(list_substance)):
...
def operation_record_into_mongo(user, operation_type, small_operation_type, content):
    ...
    try:
        ...
        logger.debug("tool operation_record_into_mongo error:%s,user:%s, operation:%s, small_operation_type:%s, content:%s"
                     %(traceback.format_exc(e), user, operation_type, small_operation_type, content))
...
def add_rid_url_info_into_redis(rid, urls):
    ...
    try:
        ...
        logger.error('add_rid_url_info_into_redis error:%s' % traceback.format_exc(e))
...
#         return str_t
#     except Exception, e:
#         logger.debug('delete_zip error:%s' % traceback.format_exc(e))
#         return str_t
...
def judge_device_fc(devices):
    ...
    try:
        ...
        logger.error('judge_device_fc error:%s' % traceback.format_exc(e))
...
def judge_contain_chinese(str1):
    ...
    try:
        ...
        logger.error('judge contain chinese error:%s' % traceback.format_exc(e))
        return False
...
def init_dev_test(_type):
    ...
    all_list = []
    all_dict = {}
    with open('/Application/bermuda/conf/test_dev', 'r') as r:
...
def judge_dev_ForH_byip(dev_ip):
    ...
    if res:
        dev_code=res[1]
        request = urllib2.Request('http://j.hope.chinacache.com:8200/api/device/'+dev_code+'/deviceBaseInfo')
        request.add_header('X-HOPE-Authn','hope')
         #response = urllib2.urlopen(request)
        try:
...
def get_channel_code(channel):
    ...
    try:
        ...
        req = urllib2.Request(url=requrl, headers=headers, data=json.dumps(data_orig))
        #print req
        ...
        logger.debug("get channel code error {}".format(traceback.format_exc(e)))
    return code
def getcode_from_redis(domain):
    ...
    try:
        ...
        logger.debug("get code from redis error {}".format(traceback.format_exc(e)))
    return code
def get_uuid():
