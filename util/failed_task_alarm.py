...
LOG_FILENAME = '/Application/bermuda/logs/fail_details.log'
    ...
    try:
        if failed_dev:
            for dev_content in failed_dev:
                ...
                str += u'code: ' + code + u'    设备名称:' + hostname + u'    IP:' + host + u'\r\n'
    except Exception, e:
...
def retry_fail_dev(urls_list, command, dev_map, dev_id, count,isdir):
    ...
    try:
        ...
        ret, ret_faild = doloop(dev_map.values(), command, connect_timeout=1.5, response_timeout=1.5)  # dict dev_list_f
        if isdir:
    ...
    if error_result or results_faild_dic:
        if count == RETRY_COUNT:
            ...
            for fail_dev_list in fail_list:
                for k, resu in fail_dev_list.items():
                    host = resu["host"]
                    name = resu["name"]
                    code = resu["code"]
                    firstLayer = resu["firstLayer"]
...
def get_dev(dev_id):
    ...
    if len(dev_list_sup_name) > 0:
        for name, ip in dev_list_sup_name.items():
...
def retry_send(pre_time, cur_time):
    ...
    print dev_id_map
    success_list = []
    for dev_id, uid_list in dev_id_map.items():
        print uid_list
        ...
        if retry_url_list:
            ...
            # except Exception,e:
            # logger.error('retry error :%s' % traceback.format_exc(e))
    return success_list
...
def run():
    ...
    try:
        ...
        logger.debug('retry_send error:%s' % traceback.format_exc(e))
    print "---------------uid------------------"
        ...
        try:
            ...
            if config:
                ...
                email = [{"username": ref_err.get('username', ''), "to_addrs": config.get('Monitor_Email', []),
                          "title": u'刷新失败任务', "body": emailResult}]
    ...
    for email in emails.values():
        ...
        for em in email:
            try:
                send_imp(em.get("to_addrs"),em.get("title").encode("UTF-8"),em.get("body").encode("UTF-8"))
            except Exception,e:
                logger.warning('seng to imp error:%s' % traceback.format_exc(e))
    os._exit(0)
