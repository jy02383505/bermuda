...
LOG_FILENAME = '/Application/bermuda/logs/autodesk.log'
...
def entrance(url_result, different_time):
    ...
    if devices:
        result = doloop(devices.values(), url, different_time)
    insert_content = {}
    ...
    try:
        ...
        logger.debug('insert device_autodesk and udpate dev_autodesk error:%s' % traceback.format_exc(e))
...
def test_entrance():
    ...
    if devices:
        result = doloop(devices.values(), url, 300)
