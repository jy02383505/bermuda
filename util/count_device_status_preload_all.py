...
# LOG_FILENAME = '/Application/bermuda/logs/preload_count_device.log'
...
# 生成的预加载信息xml文件存储路径
LOG_DIR = '/Application/bermuda/logs/outputxls/preload/'
    ...
    def build_data_structure(self):
        ...
        for preload_url in preload_url_info:
            ...
            for dev in devices:
                ...
                # print "kkkkkkkkkkkk"
                # print "dev:%s" % dev
                if devices[dev]['name'] not in self.devices_info_list.keys():
        ...
        self.devices_info_list = OrderedDict(sorted(
            self.devices_info_list.items(), key=lambda t: t[1]['failed_total'], reverse=True))
...
def write_devices_failed_top(data):
    ...
    trs = []
    mrtg = ""
    for i, device in enumerate(data['data'].values()[:10]):
        try:
...
def write_user_top_for_count(date_str):
    ...
    for tab in ["username_count_list"]:
        if tab in data.keys() and (len(data[tab]) > 0):
