...
def get_crl(cert):
    ...
    if d.has_key('crlDistributionPoints')==True:
...
def get_DNS(cert):
    ...
    '''
    d=extension_analysis(cert)
    if d.has_key('subjectAltName')==True:
...
def get_upper_cert(url):
    ...
    try:
        ...
        logger.debug('-----get_upper_cert error is %s'%(traceback.format_exc(e)))
        return None
...
def all_chain(chain):
    ...
    while True:
        try:
            ...
            logger.debug('-----all_chain error is %s'%(traceback.format_exc(e)))
            return 3
...
def chain_adjustment(crt_strs):
    ...
    try:
        ...
        logger.debug('-----chain_adjustment error is %s'%(traceback.format_exc(e)))
