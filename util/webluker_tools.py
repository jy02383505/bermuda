...
# LOG_FILENAME = '/Application/bermuda/logs/webluker_tools.log'
...
def post_data_to_webluker(data, task_all, r_id):
    ...
    try:
        ...
        logger.debug('get config error:%s' % traceback.format_exc(e))
    ...
    try:
        ...
        req = urllib2.Request(url=requrl, data=data_post, headers={'Content-type': 'application/json'})
        res_data = urllib2.urlopen(req, timeout=10)
        res = res_data.read()
        logger.debug("post data to webluker  data:%s, res:%s" % (data, res))
    except Exception, e:
        logger.debug('webluker urlopen  error:%s' % traceback.format_exc(e))
    ...
    if res:
        try:
            ...
            logger.debug('post_data_to_webluker error %s' % traceback.format_exc(e))
    ...
    try:
        ...
        logger.debug('insert task_forward error task_id:%s, error:%s' %(message.get('r_id'), traceback.format_exc(e)))
    return message
...
def get_domain_from_url(url):
    ...
    try:
        ...
        logger.debug('get_domain_from_url error:%s, url:%s' % (traceback.format_exc(e), url))
        return ""
