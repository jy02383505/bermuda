...
import traceback
import requests
LOG_FILENAME = '/Application/bermuda/logs/link_portal_cms.log'
...
url_cms_check = config.get('cert_trans', 'url_cms_check')
...
url_cms_delete = config.get('cert_trans', 'url_cms_delete')
...
url_portal_delete = config.get('cert_trans', 'portal_callback')
...
def cert_portal_delete(certid_list,method ="DELETE"):
    ...
    for i in range(RETRY_NUM):
        try:
            ...
            #req = urllib2.Request(url, headers=send_headers)
            values = {"cert_ids": certid_list,'status':'DELETE'}
            jdata = json.dumps(values)
            request = urllib2.Request(url, jdata,headers=send_headers)
            #request.add_header('Content-Type', 'application/json')
            #request.get_method = lambda: 'DELETE'  # 设置HTTP的访问方式#201812
