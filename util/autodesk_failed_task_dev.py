...
LOG_DIR_ALL = '/Application/bermuda/logs/custom_all/'
LOG_DIR_FAILED = '/Application/bermuda/logs/custom_failed/'

LOG_FILENAME = '/Application/bermuda/logs/autodesk_failed_task_dev.log'
...
def parse_data(str_id, username, created_time):
    ...
    for res in result:
        ...
        try:
            if dev_result:
                devices = dev_result.get("devices").values()
                if devices:
                    # return_r[url] = []
                    for dev in devices:
                        name = dev.get("name")
            ...
            logger.debug("autodesk_failed_task_dev error:%s" % traceback.format_exc(e))
            return
        ...
        try:
            ...
            logger.debug("parse_data  sub center error:%s" % traceback.format_exc(e))
...
def parse_data_last(str_id, username, created_time):
    ...
    try:
        for res in result:
            ...
            devices = get_all_hpcc_dev(dev_result.get('devices').values())
            ...
            # progress fc
            devices_fc = get_all_fc_dev(dev_result.get('devices').values(), code='code')
            if retry_branch_id:
            ...
            # progress unknown
            devices_unknow = get_all_fc_dev(dev_result.get('devices').values(), code='code', type_dev='unknown')
            if retry_branch_id:
        ...
        logger.debug('parse_data_last formation map_result error:%s' % traceback.format_exc(e))
        return {}
...
def get_retry_branch_dev(retry_branch_id):
    ...
    try:
        ...
        logger.info('get_retry_branch_dev  retry_branch_id:%s, error:%s' % (retry_branch_id, traceback.format_exc(e)))
        return {}
...
　　　
    ...
    try:
        ...
        logger.debug("get_content_string  error:%s" % traceback.format_exc(e))
        return "email error"
...
def get_all_hpcc_dev(devices, type_dev='HPCC'):
   ...
   try:
       ...
       logger.debug('get_all_hpcc_dev parse devices error:%s' % traceback.format_exc(e))
       return result
...
def get_all_fc_dev(devices, code='code', type_dev='FC'):
   ...
   try:
       ...
       logger.info('get_all_hpcc_dev parse devices error:%s' % traceback.format_exc(e))
       return result
...
def get_all_report_hpcc_dev(_id):
    ...
    try:
        ...
        logger.debug("get_all_report_hpcc_dev error:%s" % traceback.format_exc(e))
        return result_r
...
def judge_result(devices, result_dev, type_t='url'):
    ...
    try:
        ...
        logger.debug('judge_result error:%s' % traceback.format_exc(e))
        return devices
...
def judge_result_fc(devices, result_dev, type_t='url'):
    ...
    try:
        ...
        logger.debug('judge_result_fc error:%s' % traceback.format_exc(e))
        return devices
...
def parse_result(judge_result):
    ...
    try:
        ...
        logger.debug('parse_result error:%s' % traceback.format_exc(e))
...
def get_email_to_list(username, address="email_address"):
    ...
    try:
        ...
        logger.debug('autodesk_failed_task_dev get_email_to_list error:%s' % traceback.format_exc(e))
    if result:
...
def trans_timestamp_to_str(timestamp):
    ...
    try:
        ...
        logger.debug("trans timestamp error:%s" % traceback.format_exc(e))
        return timestamp
...
def store_failed_urls(content, timestamp):
    ...
    try:
        ...
        logger.debug('store_failed_urls error:%s' % traceback.format_exc(e))
        return
