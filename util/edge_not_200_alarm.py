...
LOG_FILENAME = '/Application/bermuda/logs/device_link.log'
...
def statistic_date(result):
            ...
            if devices:
                for dev in devices:
                    ...
                    if name and code:
                        ...
                        if name not in result_map.keys():
                            code_temp = {}
                            code_temp[code] = 1
    ...
    if result_map_new:
        try:
            db.device_link_hours_result.insert(result_map_new.values())
...
def parse_result_map(result_map, count=3):
    ...
    result_list = {}
    if result_map:
        for key_name, val in result_map.items():
...
def assemble_mail_content_send(result_dict, begin_time, end_time, dev_all_number, dev_failed_number, threshold, email_to):
    ...
    if result_dict:
        for key, values in result_dict.items():
            ...
            if values:
                for key_code, value_count in values.items():
                    if key_code != '200':
