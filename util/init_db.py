...
import traceback, logging,urllib
...
def init_preload_user():
    try:
        ...
        users = json.loads(urllib.urlopen(CUSTOMERS_URL).read())
        ...
        logger.warning('init_user work error:%s' % traceback.format_exc(e))

def init_rcms_channel():
    try:
        logger.debug("init_rcms_channel begining...")
        CUSTOMERS_URL = RCMS_ROOT + "/channels"
        channels = json.loads(urllib.urlopen(CUSTOMERS_URL).read())
        db.rcms_channel.remove()
        db.rcms_channel.insert(channels)
        logger.debug("init_rcms_channel end.")
    except Exception,e:
        logger.warning('init_rcms_channel work error:%s' % traceback.format_exc(e))
...
def init_rcms_user_enabled():
    try:
        ...
        logger.warning('rcms_user_enabled work error:%s' % traceback.format_exc(e))
