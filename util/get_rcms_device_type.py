...
def assemble_node_info_new():
    ...
    try:
        ...
        logger.debug("assemble_node_info_new error:%s" % traceback.format_exc(e))
        return {}
...
def assemble_original_info(res):
    ...
    try:
        ...
        logger.debug("assemble_original_info error:%s" % traceback.format_exc(e))
        return map_re
...
def get_md5(str_1):
    ...
    try:
        ...
        logger.debug("get_md5 error:%s" % traceback.format_exc(e))
        ...
        try:
            if "appList" in dev:
                ...
                try:
                    ...
                    logger.error(ex)
            try:
                DEVICE_IP.hmset("DEVICE_NAME_IP", {dev.get("devName"): dev.get("devAdminIP")})
                DEVICE_IP.hmset("DEVICE_IP_NAME_IP", {dev.get("devAdminIP"): dev.get("devName")})
            except Exception, ex:
                logger.error(ex)
        except:pass
    # logger.debug(devapp)
