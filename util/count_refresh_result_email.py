...
def get_top10_device(fail):
    ...
    global refresh_result
    if fail:
        sort_list=sorted(refresh_result.iteritems(), key=lambda d: d[1].get('514',0), reverse=True)
    else:
        sort_list=sorted(refresh_result.iteritems(), key=lambda d: d[1]['total'], reverse=True)
    top_list=[]
    for i in xrange(10):
        if i < len(sort_list):
            dev_dict={}
