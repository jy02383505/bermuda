...
# LOG_FILENAME = '/Application/bermuda/logs/webluker_username_url.log'
...
LOG_FILENAME = '/Application/bermuda/logs/webluker_username_url_up.log'
...
def get_data():
    ...
    try:
        ...
        res_data = urllib2.urlopen(req, timeout=100)
        res = res_data.read()
        logger.debug('get data from webluker res:%s' % res)
        return json.loads(res)
    except Exception, e:
        logger.debug('get data from webluker error:%s' % traceback.format_exc(e))
        return {}
    # print res
...
def parse_data(webluker_data):
    ...
    try:
        ...
        logger.debug('parse_data  error:%s' % traceback.format_exc(e))
