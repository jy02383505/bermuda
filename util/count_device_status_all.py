...
from rep_refresh_count import get_refresh_num,get_fail_dev_num
...
null = ''
source_device_dict = {}
LOG_DIR = '/Application/bermuda/logs/outputxls/'
LOG_FILENAME = '/Application/bermuda/logs/count_device.log'
...
ads_to = ['huan.ma@chinacache.com', 'CPRD-HPCC-OM@chinacache.com', 'chunyang.wang@chinacache.com', 'rong.chen@chinacache.com', 'BRE-OPS-WD@chinacache.com',
          ...
          'zhida.piao@chinacache.com', 'feng.qiu@chinacache.com','tao.pang@chinacache.com','yu.zhao@chinacache.com'
          ]
...
def get_data(connect, date_day, date_month, date_year, h, real_date):
    ...
    for doc in connect.find({'created_time': {'$gte': leftend, '$lt': rightend}}):
        for device in doc['devices'].keys():
            ...
            if h not in source_device_dict[device].keys():
    ...
    for dev in devices:
        try:
            ...
            try:
                ...
                got_list = eval(urllib.urlopen(req, timeout=100).read())
...
def get_firstLayerDevices():
    try:
        got_list = eval(urllib.urlopen(RCMS_DEVICES_LAYER, timeout=100).read())
        dev_info = {}
...
def start_thread(connect, date_day, date_month, date_year, devices_type, name_dict, rcmstime_time, real_date):
    ...
    devices_type = get_device_type(name_dict.keys())
    name_of_devices = OrderedDict(
        sorted(name_dict.items(), key=lambda t: (t[1]['501'] + t[1]['502'] + t[1]['503']), reverse=True))
    # print name_of_devices
...
def process(date_str):
    ...
    if not get_dat:
        ...
        name_of_devices = OrderedDict(
            sorted(name_dict.items(), key=lambda t: int(t[1]['501']) + int(t[1]['502']) + int(t[1]['503']),
                   reverse=True))
...
def write_top(data):
    ...
    for i, device in enumerate(data['data'][:10]):
        try:
            ...
            mrtg_info = eval(urllib.urlopen(req, timeout=100).read())
...
def script_run():
    ...
    try:
        ...
        content += '<br/><br/>刷新回调失败设备top10:'
        content += get_fail_dev_num()
    except:
        pass
