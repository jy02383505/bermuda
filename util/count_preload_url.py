...
LOG_FILENAME = '/Application/bermuda/logs/count_preload.log'
...
def write_log(end_date, content):
    ...
    log_file = '/Application/bermuda/logs/preload_count_%s' % file_name
    f = file("%s" % (log_file), "a")
    f.write(content + "\r\n")
    f.flush()
    f.close()
...
class Preload_Count(object):
    ...
    def count_query(self):
        ...
        for query in self.querys:
            ...
            # {'duwan':1,'duwan1':1}
            if u_name not in self.u_dic.keys():
                self.u_dic[u_name] = 1
            else:
                self.u_dic[u_name] += 1
