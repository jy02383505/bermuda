...
LOG_FILENAME = '/Application/bermuda/logs/refresh_count.log'
...
class RefreshCount(object):
    ...
    def write_log(self, content):
        ...
        log_dir = '/Application/bermuda/logs/refresh_count'
    ...
    def count_query_des(self):
        ...
        for query in self.querys:
            ...
            if u_name not in self.usernames:
                ...
                self.usernames_count[u_name] += 1
            if status  in ["FAILED"]:
                if u_name not in self.usernames_fail_count.keys():
        ...
        self.usernames_count = sorted(self.usernames_count.items(), key=lambda t: t[1], reverse=True)
        self.usernames_fail_count = sorted(self.usernames_fail_count.items(), key=lambda t: t[1], reverse=True)
    ...
    def count_user_channels(self):
        user_num_dic = {}
        channels_num = 0
        for username, channels in self.user_channels.items():
            if username not in user_num_dic.keys():
