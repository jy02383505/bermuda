...
import traceback
LOG_FILENAME = '/Application/bermuda/logs/staticsitc_preload.log'
...
class PreloadCount(object):
    ...
    def count_names_des(self):
        ...
        self.usernames_count = sorted(
            self.usernames_count.items(), key=lambda t: t[1], reverse=True)
    ...
    def count_finished_names(self):
        ...
        for res in result:
            u_name = res.get("username")
            if u_name not in self.usernames_finished_count.keys():
        ...
        self.usernames_finished_count = sorted(
            self.usernames_finished_count.items(), key=lambda t: t[1], reverse=True)
    ...
    def count_progress_names(self):
        ...
        for res in result:
            u_name = res.get("username")
            if u_name not in self.usernames_progress_count.keys():
        ...
        self.usernames_progress_count = sorted(
            self.usernames_progress_count.items(), key=lambda t: t[1], reverse=True)
    ...
    def judge_task_success_failed(self, dev_id, retry_branch_id):
        ...
        if dev_id:
            try:
                ...
                logger.debug("get data error:%s" % traceback.format_exc(e))
                return False
            if not preload_dev:
                return False
            try:
                ...
                logger.debug(
                    'judge_tak_success_failed parse data error:%s' % traceback.format_exc(e))
                return False
    ...
    def get_data_from_retry_device_branch(self, retry_branch_id):
        ...
        if not retry_branch_id:
            ...
            try:
                ...
                logger.debug(
                    'get_data_from_retry_device_branch parse data error:%s' % traceback.format_exc(e))
