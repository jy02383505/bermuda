...
import logging, zlib
from asyncpostal import HttpClient,HttpClient_url
...
def doloop_url(devs, tasks, Interface,connect_timeout=5, response_timeout=8):
    ...
    for dev in devs:
        clients.append(HttpClient_url(dev['host'], port, Interface, command, len(devs) * 200, my_map, connect_timeout,response_timeout))
    ...
    for c in clients:
        ...
        if response_body:
            try:
                ...
                logger.debug ("pre_host_test:%s, response_code_test:%s, response_body:%s" % (c.host, response_code, response_body))
            except Exception, e:
...
def process_loop_ret(http_results, dev_map):
    ...
    for result in http_results:
        try:
            ...
            logger.debug("transfer_cert_postal process_loop_ret result has problem:%s, error:%s" % (result, traceback.format_exc(e)))
