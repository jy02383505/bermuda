...
#                 except Exception, e:
#                     logger.debug('link_detection_refresh redis5 error:%s' % traceback.format_exc(e))
#         except Exception, e:
...
def send_subcenter(rid, failed_dev_list, branch_center_list, path, edge_port):
    ...
    for dev in branch_center_list:
        try:
            ...
            port = int(dev_temp[1])
            clients.append(
                HttpClient(host, port, json.JSONEncoder().encode(post_data), len(branch_center_list) * 200, my_map,
                           connect_timeout, response_timeout, path))
        except Exception, ex:
            logger.error(ex)
...
def get_failed_dev(failed_dev_list, type_t="FC"):
    ...
    try:
        ...
        logger.debug('get_failed_dev error %s, failed_dev_list:%s, type_t:%s' % (traceback.format_exc(e),
...
def link_detection_preload(urls, failed_dev_list):
    try:
        ...
        branch_centers = get_subcenters()
        branch_center_list = branch_centers.values() if branch_centers else []
        ...
        logger.debug("link_detection_preload[error]: %s" % (traceback.format_exc(e)))
