...
multi_db = database.multi_session()
...
'''
    ...
    for key,value in devices.items():
        ...
        for url in urls:
            ...
            if url.get("physical_del_channel",False):
                uu["physical_del_channel"] = url.get("physical_del_channel",False)
            # url_list.append(uu)
            dev_map["url"] = uu
    ...
    try:
        ...
        #     multi_db.device_urls_day.insert_many(dev_map_list)
        dev_dict = {}
        for dev_m in dev_map_list:
            dev_key = hash(dev_m.get("hostname"))%50
            dev_dict.setdefault(dev_key,[]).append(dev_m)

        db_key = "device_urls_day_{}"
        for db_num, db_value in dev_dict.iteritems():
            db_name = db_key.format(db_num)
            multi_db[db_name].insert_many(db_value)
...
def update_device_count(monitor_map):
    ...
    try:
        ...
        for _code , _info in monitor_map.items():
            r_key = '%s_%s' %(day_key, _code)
            r_first_key = '%s_%s_first' %(day_key, _code)
            for _hostname, _count in _info.items():
