    ...
    for c in clients:
        ...
        if response_body:
            try:
                ...
                logger.debug ("pre_ip_test:%s, response_code_test:%s, response_body:%s" % (c.host, response_code, response_body))
            except Exception, e:
...
def process_loop_ret(http_results, dev_map):
    ...
    for result in http_results:
        try:
            ...
            logger.debug("cert_query_postal process_loop_ret result has problem:%s, error:%s" % (result, traceback.format_exc(e)))
