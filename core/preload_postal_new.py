...
def doloop(devs, urls, connect_timeout=5, response_timeout=8):
    ...
    for dev in devs:
        ...
        if have_first_layer:
            if dev.get('firstLayer'):
                clients.append(HttpClient(dev.get('host'), port, get_command_json(urls, urls[0].get("action"), dev, check_conn=True),len(devs) * 200, my_map, connect_timeout, response_timeout))
            else:
                clients.append(HttpClient(dev.get('host'), port, get_command_json(urls, urls[0].get("action"), dev), len(devs) * 200, my_map, connect_timeout, response_timeout))
        else:
            clients.append(HttpClient(dev.get('host'), port, get_command_json(urls, urls[0].get("action"), dev, check_conn=True),len(devs) * 200, my_map, connect_timeout, response_timeout))
    ...
    for c in clients:
        ...
        if response_body:
            try:
                pre_ret.append(c.host + '\r\n' + response_body.split('\r\n\r\n')[1] + '\r\n%d\r\n%.2f\r\n%.2f\r\n%.2f' % (
                    response_code, total_cost, connect_cost, response_cost))
                # logger.warn("devs: %s doloop response_body: %s" % (devs, response_body))
                logger.debug("doloop pre_host_test: %s|| response_code_test: %s|| response_body: %s" % (c.host, response_code, response_body))
            except Exception, e:
                logger.error("doloop pre_devs: %s|| response_body error: %s" % (devs, response_body))
                logger.error("doloop error: %s" % traceback.format_exc(e))
        else:
            # handling the case without response_body
            pre_ret_faild.append(c.host + '\r\n%d\r\n%.2f\r\n%.2f\r\n%.2f' % (
                    response_code, total_cost, connect_cost, response_cost))

    # logger.debug("doloop -> clients: %s, pre_ret: %s, pre_ret_faild: %s" % (clients, pre_ret, pre_ret_faild))
...
def process_loop_ret(http_results, dev_map):
    ...
    for result in http_results:
        try:
            ...
            logger.debug("process_loop_ret result has problem: %s|| error: %s" % (result, traceback.format_exc(e)))
