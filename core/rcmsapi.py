...
# LOG_FILENAME = '/Application/bermuda/logs/rcmsapi.log'
...
def getGrayDevices(channel_code):
    if not channel_code:
        return []
    return redisutil.read_grayDevices(channel_code)
...
def getDevStatus(devName):
    return json.loads(urllib.urlopen(DEVICE_STATUS_URL % devName).read())

def getBUDepartment(devName):
    return json.loads(urllib.urlopen(DEVICE_BU_DEPARTMENT % devName).read())

def getChannelCode(channelName):
    return json.loads(urllib.urlopen(CHANNELNAME % channelName).read())
...
def isValidChannelByPortal(username, channel_name):
    ...
    try:
        if redisutil.portal_cache.hexists(key, channel_name):
            ...
            user = json.loads(urllib.urlopen(PORTAL_CHANNEL % str(username), timeout=10).read())
