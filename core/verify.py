...
import sendEmail,httplib, urllib, base64, database,urllib2,traceback,hashlib
    ...
    for url in urls:
        try:
            try:
                ...
                logger.error('db_update error url id %s :%s' % (ObjectId(url.get("id")), ex))
            ...
            if not db_request.get("unprocess"):
                request_status = 'FAILED' if query_db_session.url.find({'r_id':r_id,'status':'FAILED'}).count()  else 'FINISHED'
                try:
                    db_update(db.request, {"_id": r_id},
                              {"$set": {'status': request_status, 'finish_time': datetime.now()}})
                except Exception, ex:
                    logger.error('db_update error request rid %s :%s' % (r_id, ex))
                logger.debug("verify db successed, r_id = %s request_status = %s " % (url.get("r_id"),request_status))
                if db_request.get('callback'):
                    if url.get('parent') == 'snda':
                        snda_callback(db_request.get('callback'), url.get("r_id"), url.get('username'))
                    else:
                        db_request["finish_time"] = datetime.now()
                        # callback(db_request,query_db_session) # 20190513
                        try:
                            callback_celery.delay(db_request)
                        except Exception,ex:
                            pass
        except Exception,e:
...
def snda_callback(callback_info, r_id, username):
    ...
    try:
       ...
       # logger.debug('snda callback url %s' %(callback_url))
        ...
        r = urllib.urlopen(callback_url , urllib.urlencode(data))
        logger.debug('snda callback res %s' %(r.read()))
    except Exception, e:
        logger.debug("snda callback error!,  %s" % (traceback.format_exc(e)))

@task(ignore_result=True, default_retry_delay=10, max_retries=3)
def callback_celery(request):
    db = query_db_session
    try:
            ...
            if callback.get('email'):
                email = [
                    {"username": request.get('username'), "to_addrs": callback.get('email'), "title": 'refresh callback',
                     "body": emailResult}]
                #queue.put_json2('email', email)#20181127 not callback email
        ...
        if callback.get('url'):
            ...
            if status != 200:
                for i in range(3):
                    status = doPost(callback.get('url'),urlResult)
                    logger.debug('request : %s ,urlcallback retry count:%s ,status:%s.' % (request.get('_id'), i, status))
                    if status == 200:
                        break
        if callback.get('ntease_itemid'):
            result = '<?xml version=\"1.0\" encoding=\"utf-8\" ?><fwif><item_id>%s</item_id><op_result>SUCCESS</op_result><detail>SUCCESS</detail></fwif>' % callback.get('ntease_itemid')
            headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/xml", "Authorization": "Basic " + str.strip(base64.encodestring(USERNAME_NTESE + ':' + PASSWORD_NTESE))}
            hc = httplib.HTTPConnection(NTESE_DOMAIN, NTEST_PORT, timeout=4)
            params = urllib.urlencode({'content': result.encode('utf-8')})
            hc.request('POST', '/cdnreport/', params, headers)
            hc.close()
    except:
        logger.debug("r_ud:%s do callback error, callback body :%s." %(request.get("_id"),request.get("callback")))
...
def doPost(url,urlResult):
    ...
    try:
        request = urllib2.Request(url, urlResult, headers=headers)
        response = urllib2.urlopen(request, timeout=7.5)
        logger.debug("do_post_json url:%s data: %s, code :%s." %(url, urlResult, response.getcode()))
        return response.getcode()
    except Exception, e:
        logger.debug("do_post_json url:%s error: %s. " %(url, traceback.format_exc(e)))
        return 0
...
def get_callback(request,db):
    ...
    urlResult = {"request_id": str(request.get('_id')), "CustID": request.get("username"), "Submission time": str(request.get("created_time")), "Completion time": str(request.get("finish_time")), "URL count": len(urls), "Content purged": {"urls": []}}
    for url in urls:
