# -*- coding:utf-8 -*-
import os, copy, time, datetime, base64, traceback, random, md5, requests, Queue
import socket
...
CERT_Name_CACHE = redisfactory.getDB(9)
...
#中央生成zip位置
CERT_DIR = '/Application/bermuda/static/certs/'
ERROR_HOSTS_DIR = '/Application/bermuda/logs/cert_failed/'
...
def dispatch(tasks):
    ...
    try:
        ...
        logger.debug("dispatch cert trans error:%s " % traceback.format_exc(e))
...
def init_cert_devs(send_devs):
    ...
    if send_devs == 'all_hpcc':
        ...
        if WORKER_HOST in ["BGP-SM-e-3gN", "BGP-SM-e-3gK", "BGP-SM-e-3g9"]:
            all_devs = [{'name': 'BGP-SM-e-3gN', 'host': '223.202.203.55', 'type': 'make_test'},
                        {'name': 'BGP-SM-e-3gK', 'host': '223.202.203.52', 'type': 'make_test'},
                        {'name': 'BGP-SM-e-3g9', 'host': '223.202.203.31', 'type': 'make_test'}]
            devs_dict = {'BGP-SM-e-3gN': {'name': 'BGP-SM-e-3gN', 'host': '223.202.203.55', 'type': 'make_test'},
                         'BGP-SM-e-3gK': {'name': 'BGP-SM-e-3gK', 'host': '223.202.203.52', 'type': 'make_test'},
                         'BGP-SM-e-3g9': {'name': 'BGP-SM-e-3g9', 'host': '223.202.203.31', 'type': 'make_test'}}
        else:
        ...
        raise
    dev_id = s1_db.cert_trans_dev.insert({'devices':devs_dict, 'created_time':datetime.datetime.now(), 'unprocess': len(all_devs)})
...
def get_all_hpcc_devs():
        ...
        for d in all_info['data']:
            if d['device_group'] in ['H_D_CACHE', 'H_P_CACHE', 'H_V_CACHE'] or 'Single' in d['device_group']:
                ...
                if d['deviceState'] == "Online":
                    _dev = {'name':d['device_name'], 'host':d['ip'],'code':0,'type':d['device_group']}
                    res.append(_dev)
    ...
    if not res:
        ...
        res = [json.loads(i) for i in _cache.values()]
        res_dict = {i:json.loads(v) for i,v in _cache.items()}
...
def get_all_hpcc_devs_by_rcms():
    ...
    if all_devs:
        ...
        if cache_dict:
            ...
            CERT_TRANS_CACHE.hmset(cache_key, cache_dict)
    t3 = time.time()
    logger.debug("all_hpcc devs rcms filter cost %s seconds all_devs count %s"%(t3-t2, len(res)))
...
def worker(tasks, send_devs):
    ...
    try:
        ...
        #make_all_callback_force([i['_id'] for i in tasks])
        logger.debug('cert trans worker error is %s' %(traceback.format_exc(e)))
...
def check_cert_task(task_ids):
    ...
    callback_list = s1_db.cert_trans_tasks.find({'_id': {'$in':[ObjectId(task_id) for task_id in task_ids]}, 'status':'FINISHED'})
    #尝试2次回调
    for c in callback_list:
        if not c['rcms_callback_time'] or not c['portal_callback_time']:
            ...
            logger.debug('---check_cert_task  retry callback task_id %s  cert_id %s---'%(c['_id'], c['c_id']))
            make_all_callback(c, cert_info)

    task_info_list = s1_db.cert_trans_tasks.find({'_id': {'$in':[ObjectId(task_id) for task_id in task_ids]}, 'status':'PROGRESS'})
...
def update_db_dev(dev_id, results):
    ...
    try:
        ...
        s1_db.cert_trans_tasks.update_many({'dev_id':ObjectId(dev_id)}, {'$set':{'finish_time': now}})

    except Exception, e:
        logger.debug('cert update_db_dev error is %s' %(traceback.format_exc(e)))
    ...
    try:
        ...
        if dev_map:
            ...
            try:
                ...
                retry_results = cert_trans_postal.retry(dev_map.values(), tasks, results_faild_dic)
            except Exception,ex:
                retry_results = []
...
def get_custom_seed(username):
    ...
    if not seed:
        ...
        if seed_info:
            ...
            try:
                db.cert_user_seed.insert_one({'username':username, 'seed':seed, 'created_time': datetime.datetime.now()})
...
def make_result_cache(task_id, dev_dict):
    ...
    for k,v in dev_dict.items():
        save_cache[v['host']] = json.dumps({'host': v['host'], 'result_status': 0, 'type': v['type'], 'name': v['name']})
...
def save_result(data_list, remote_ip):
	...
	logger.debug('save_result task_id %s  status:%s remote_ip %s'%(task_id, status, remote_ip))
	...
	logger.debug('save_result hget cache_key %s  ,remote_ip %s'%(res_cache_key, remote_ip))
	    ...
	    before_success_num = CERT_TRANS_CACHE.scard(success_cache_key)
            logger.debug('save_result add count success remote_ip %s before_success_num %s'%(remote_ip, before_success_num))
	...
	logger.debug('save_result remote_ip %s success_num %s, failed_num %s, all_count %s' %(remote_ip, success_num, failed_num, all_dev_num))
...
def set_finished(task_id, status):
    ...
    '''
    now = datetime.datetime.now()
    task_info = s1_db.cert_trans_tasks.find_one_and_update({'_id':ObjectId(task_id),'status':"PROGRESS"},{"$set":{'status':status,'hpc_finish_time':now}}, return_document=ReturnDocument.AFTER)
    #cache -> mongo
    res_cache_key = '%s_res'%(task_id)
    dev_num_cache_key = '%s_res_dev_num'%(task_id)
    failed_cache_key = '%s_res_failed'%(task_id)
    ...
    if unprocess <= 0:
        unprocess = 0
    save_cache = {'_id': ObjectId(task_id), 'devices': {}, 'created_time': now, 'unprocess': int(unprocess)}
    ...
    try:
        ...
        logger.debug('set_finished error is %s' %(traceback.format_exc(e)))
        return
...
def make_dir_name(username):
    ...
    _md5 = md5.new()
    _md5.update('%s_%s_%s'%(username, num1,num2))
    _md5_num = _md5.hexdigest()
    name = _md5_num + '_' + t_s
    return name
...
def get_all_cert_work():
    key_name = "valid_cert_name_list"
    name_list = CERT_Name_CACHE.smembers(key_name)
    name_list = list(name_list)
    if not name_list or len(name_list) < 600:
        all_cert = s1_db.cert_detail.find({'t_id': {"$exists": False}}, {"save_name": 1, "_id": 0})
        name_list = [name.get("save_name") for name in all_cert]
        name_list_set = set(name_list)
        CERT_Name_CACHE.sadd(key_name, *name_list_set)
        CERT_Name_CACHE.expire(key_name, 10*24*60*60)
    return name_list

def add_certName_redis(save_name):
    key_name = "valid_cert_name_list"
    CERT_Name_CACHE.sadd(key_name, save_name)

def remove_certName_redis(save_name):
    key_name = "valid_cert_name_list"
    CERT_Name_CACHE.srem(key_name, save_name)

def get_part_cert(cert_list):
    '''
    获取所有证书
    cert p_key s_name op_type cert_type task_id seed s_dir
    '''
    s_dir = config.get('cert_trans', 'cache_dir')
    all_cert = s1_db.cert_detail.find({'save_name':{'$in': cert_list}})

    all_res = []

    for cert_info in all_cert:

        _data = {'s_name':cert_info['save_name'], 'op_type': 'pull', 'cert_type': cert_info.get('cert_type', 'crt'), 's_dir': s_dir}
...
def get_all_cert_by_dev(ignore_n,ignore_transfer=False):
    ...
    s_dir = config.get('cert_trans', 'cache_dir')
    if ignore_transfer:
        all_cert = s1_db.cert_detail.find({'save_name': {'$not': {'$in': ignore_n}},'t_id':{"$exists":False}})
    else:
        all_cert = s1_db.cert_detail.find({'save_name': {'$not': {'$in': ignore_n}}})
...
def make_all_callback(task_info, cert_info):
    ...
    # rcms_callback_time = task_info.get('rcms_callback_time') #20190529
    portal_callback_time = task_info.get('portal_callback_time')
    # if rcms_callback_time and portal_callback_time: #20190529
    if portal_callback_time:
        logger.debug('----make_all_callback task_id %s  and callback portal in %s'%(task_id, portal_callback_time))
        return True

    cert_save_name = cert_info['save_name']
    hasFc = cert_info.get('hasFc',False) #20181216
    certId = str(cert_info.get('_id',0))
    rcms_body = {'ROOT':{'HEADER':{'AUTH_INFO':{'LOGIN_NO':'refresh_preload','LOGIN_PWD':'cert_2018_Q2','FUNC_CODE':'9040'}}, 'BODY':{'BUSI_INFO':{'CRT_INFO':{'extnId': task_info['c_id'],'crtName':task_info['cert_alias'], 'userId': str(task_info.get('user_id', 2275)),'isSafe': (lambda x : 'true' if x ==False else 'false') (cert_info.get('middle_cert_lack',False)),'crtFilename':'%s.crt'%(cert_save_name), 'keyFilename': '%s.key'%(cert_save_name) }  }}}}
    ...
    #     if rcms_success:
    #         s1_db.cert_trans_tasks.find_one_and_update({'_id':ObjectId(task_id),'rcms_callback_time':""},{"$set":{'rcms_callback_time':now}})
    #         to_portal = True
    #         logger.debug('----make_all_callback task_id %s cert_id %s callback rcms success in %s'%(task_id, task_info['c_id'], now))
    # else:
    #     to_portal = True
    ...
    if hasFc == False:
        if to_portal and not portal_callback_time:
            ...
            portal_success = do_callback_portal(PORTAL_URL, 'portal', {'cert_ids': [certId],'status':'ENABLE'})
            if portal_success:
                s1_db.cert_trans_tasks.find_one_and_update({'_id':ObjectId(task_id),'portal_callback_time':""},{"$set":{'portal_callback_time':now}})
                logger.debug('----make_all_callback task_id %s cert_id %s callback portal success in %s'%(task_id, task_info['c_id'], now))
                return True
...
def do_callback_portal(url, _type, command ):
    ...
    for x in xrange(2):
        ...
        try:
            ...
            logger.error(
                "%s callback error command is %s, error is %s" % (_type, json_command, traceback.format_exc(e)))
            continue
...
def do_callback(url, _type, command ):
    ...
    for x in xrange(2):
        ...
        try:
            ...
            logger.error("%s callback error command is %s, error is %s" % (_type, json_command,traceback.format_exc(e) ))
            continue
...
def get_error_dev_result(task_info):
    ...
    for k, v in res_cache.items():
        v_obj = json.loads(v)
        if v_obj['result_status'] != 200:
            error_list.append({'host':v_obj['host'], 'code': v_obj['result_status'], 'name': v_obj['name'], 'type': v_obj['type']})
    return error_list
...
def send_error_email(task_info, error_list):
    ...
    #下发
    for h_k, h_d in dev_info['devices'].items():
        if h_d['code'] == 200:
            continue
        table += '<tr><td>%s</td><td style="text-align:center">%s</td><td style="text-align:center">%s</td><td style="text-align:center">%s</td><td style="text-align:center">%s</td><td style="text-align:center">%s</td><td style="text-align:center">%s</td></tr>'%(h_k, h_d['host'], h_d['type'], h_d['code'], h_d['a_code'], h_d['r_code'], 0)
    ...
    for error_dev in error_list:
        ...
        if e_code in [0, 524]:
            continue
        table += '<tr><td>%s</td><td style="text-align:center">%s</td><td style="text-align:center">%s</td><td style="text-align:center">%s</td><td style="text-align:center">%s</td><td style="text-align:center">%s</td><td style="text-align:center">%s</td></tr>'%(error_dev['name'], error_dev['host'], error_dev['type'],200, 200, 200, error_dev['code'])
    ...
    try:
       #个人版
        ...
        if p_attch:
            ...
            p_attchs = []
        sendEmail.send(['pengfei.hao@chinacache.com'], '自助证书下发邮件详情', all_content.encode('utf-8'), 'html', p_attchs)
...
def make_op_log(username, remote_ip, cert_ids, _type):
    ...
    生成操作日志
    '''
    s1_db.cert_op_log.insert({"username":username, "remote_ip":remote_ip, "cert_ids":cert_ids, "type": _type, "created_time": datetime.datetime.now()})
...
def store_error_host(cert_id, error_list):
    ...
    try:
        ...
        logger.debug('store_all_urls error:%s' % traceback.format_exc(e))
        return None
