...
# except Exception, e:
#     logger.debug('get user_list_special error:%s' % traceback.format_exc(e))
...
def submit(refresh_task, urls):
    ...
    try:
        ...
        try:
            ...
            logger.debug('domain ignore error {}'.format(traceback.format_exc(e)))
        ...
        try:
            ...
            logger.debug('domain ignore error {}'.format(traceback.format_exc(e)))
        try:
            urls = manager_https(urls)
        except Exception,e:
            logger.debug('manger https error{}'.format(traceback.format_exc(e)))
        if not urls:
            return
        ...
        db.request.insert({"_id": refresh_task.get('r_id'), "username": refresh_task.get("username"), "parent" : refresh_task.get("parent"),
                   "callback": refresh_task.get("callback"), "status": "PROGRESS", "unprocess": len(messages)+len(messages_high),
        ...
        if web_task:
            try:
                ...
                logger.debug('splitter_new  webluker task have r_id:%s, error:%s' % (web_task.get('r_id'), traceback.format_exc(e)))
        ...
        logger.warning('submit error! do retry. error:%s' % traceback.format_exc(e))
...
def process(db, refresh_task, check_overload=False):
    ...
    Exceed = False
    try:
        logger.debug("process refresh_task:%s %s %s " % (refresh_task['r_id'], refresh_task['username'], refresh_task['urls'] if 'urls' in refresh_task.keys() else refresh_task['dirs']))
        if check_overload:
...
def init_prefix_url(tmpUrlList, u, username_t):
    ...
    try:
        ...
        logger.debug('init_prefix_url error:%s' % traceback.format_exc(e))
        # return tmpUrlList
    ...
    try:
        ...
        for _u in urls:
            if _u.get('status') == 'PROGRESS':
                init_regex_url(tmpUrlList, _u)
                # init_regex_htv(tmpUrlList, u)
        tmpUrlList_2 = copy.deepcopy(tmpUrlList)
        # do rewrite
        for u in tmpUrlList:
            if u.get('status') == 'PROGRESS':
                init_rewrite_url(tmpUrlList_2, u)
        return tmpUrlList_2
    except Exception,e:
        logger.info('regex splite error:%s' % traceback.format_exc(e))
        return urls
...
def domain_ignore(urls):
    try:
        ...
        if domain_ignore_h:
            ...
            domain_ignore_list =[channel_code for channel_code in regexDict.keys()]
        ...
        logger.debug('url change lower error {}'.format(traceback.format_exc(e)))
        return urls
def manager_https(urls):
    try:
        tmpUrlList = copy.deepcopy(urls)
        for u in urls:
            s = u.get('url').split('/', 3)
            ...
            if s[0] == 'https:':
                rewriteUrl = channelname + ':443' + method
                tmpUrlList.append({"r_id": u.get('r_id'), 'ignore_case': u.get('ignore_case'), "url": rewriteUrl,
                                   "status": u.get('status'), "isdir": u.get('isdir'), "username": u.get("username"),
                                   "created_time": datetime.now(), "action": u.get('action'),
                                   "is_multilayer": u.get('is_multilayer'), "parent": u.get("parent"), 'type': 'https',
                                   "channel_code": u.get('channel_code')})
                # re_http = "http://"+s[2]+method
                # tmpUrlList.append({"r_id": u.get('r_id'), 'ignore_case': u.get('ignore_case'), "url": re_http,
                #                    "status": u.get('status'), "isdir": u.get('isdir'), "username": u.get("username"),
                #                    "created_time": datetime.now(), "action": u.get('action'),
                #                    "is_multilayer": u.get('is_multilayer'), "parent": u.get("parent"), 'type': 'https',
                #                    "channel_code": u.get('channel_code')})
...
def dir_and_url(urls):
    try:
        ...
        if dir_and_url_h:
            ...
            dir_and_url_list =[channel_code for channel_code in regexDict.keys()]
        ...
        logger.debug('dir_and_url_list error {}'.format(traceback.format_exc(e)))
        return urls
...
def init_chinese_url(tmpUrlList, u):
    ...
    try:
        ...
        if judge_contain_chinese(u.get('url')):
            ...
            try:
                url_utf_8_encode = urllib.quote(u.get('url').encode('utf-8'), ';:/.?=_*@&+$,#()')
                url_gbk_encode = urllib.quote(u.get('url').encode('gbk'), ';:/.?=_*@&+$,#()')
                ...
                logger.debug('init_chinese_url url code error:%s' % traceback.format_exc(e))
    except Exception, e:
        logger.error('splitter_new init_chinese_url error:%s' % traceback.format_exc(e))
