...
# LOG_FILENAME = '/Application/bermuda/logs/autodesk_test.log'
        ...
        if not result:
            ...
            if refresh_request and refresh_request.get('task_id'):
                ...
                if refresh_request:
                    ...
                    result = {
                        'code': 404, 'r_id': rid, 'msg': '%s not found.' % rid
                    }
        else:
...
def refresh_get_data_from_webluker(rid):
    ...
    try:
        ...
        logger.error('refresh_get_data_from_webluker error:%s' % traceback.format_exc(e))
        #requrl = "http://223.202.202.37/nova/get/domain/refresh/status/"
        requrl = "http://api.novacdn.com/nova/get/domain/refresh/status/"
    try:
        req = urllib2.Request(url=requrl, data=data_urlencode, headers={'Content-type': 'application/json'})
        ...
        print('refresh get data from webluker error:%s' % traceback.format_exc(e))
        return {}
...
def get_search_result_by_rid_webluker_one(rid, username, rid_r):
    ...
    if refresh_request:
        ...
        result = {
            'code':404, 'r_id': rid_r, 'msg': '%s not found.' % rid_r
        }
    return result
            ...
            if refresh_request:
                ...
                if not finishedTime:
                    ...
                    try:
                        ...
                        logger.debug('get_search_result_by_rid_autodesk not judeg_status_urls error:%s' % traceback.format_exc(e))
    ...
    for rid in rid.split(','):
        ...
        if not result:
            ...
            try:
                ...
                logger.error('get_search_result_by_rid_autodesk_new get quer_type error:%s' % traceback.format_exc(e))
...
def update_url_request_autodesk(rid):
    ...
    try:
        ...
        logger.debug('update request url_autodesk from query error:%s' % traceback.format_exc(e))
...
def parse_url_data(result):
    ...
    try:
        ...
        logger.debug('parse_url_data error:%s' % traceback.format_exc(e))
