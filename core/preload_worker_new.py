...
def dispatch(urls):
    ...
    try:
        ...
        if devices_if:
            ...
            # # ---for pddlx
            # if urls[0].get('username') in ['pddlx']:
            #     dev_id_only_nf, pre_devs_only_nf, ref_devs_only_nf = init_pre_devs(urls[0].get('channel_code'), only_nf=True)
            #     logger.debug("dispatch[pddlx__only_nf] dev_id_only_nf: %s|| pre_devs_only_nf: %s" % (dev_id_only_nf, pre_devs_only_nf))

        logger.debug("dispatch dev_id: %s|| pre_devs: %s" % (dev_id, pre_devs))
        if not len(pre_devs):
            logger.error("no_devices::::%s"%urls[0].get('channel_code'))
        #  test down
        ...
        for url in urls:
            ...
            #preload_url任务缓存
            p_set_id_dispatch = PRELOAD_CACHE.set(url.get('_id'), json.dumps(
                {"dev_id": str(dev_id), "task_id": url.get("task_id"),
                 "channel_code": url.get("channel_code"), "unprocess": len(pre_devs),
                 'status': 'PROGRESS', 'check_type': url.get('check_type'),
            ...
            if is_timer_tasks:
                try:
                    ...
                    logger.debug("dispatch preload_worker update_one _id: %s|| error:%s" % (url['_id'], traceback.format_exc()))

            #设备缓存
            p_hmset_id_dispatch = PRELOAD_CACHE.hmset('%s_dev'%(str(url.get('_id'))), create_preload_result_dict_to_cache(pre_devs))
            PRELOAD_CACHE.expire(url.get('_id'), int(3600*24*5.5))
            PRELOAD_CACHE.expire('%s_dev'%(str(url.get('_id'))), int(3600*24*5.5))
            logger.debug("dispatch p_set_id_dispatch: %s|| p_hmset_id_dispatch: %s" % (p_set_id_dispatch, p_hmset_id_dispatch))
...
def nf_tasks_begin(url_id, cache_body):
    ...
    try:
        ...
        if nf_begin:
            logger.debug("nf_tasks_begin [nf_tasks_begin!!!preload_200_num(%s) >= dev_first_num(%s)] url_id: %s|| cache_body: %s" % (preload_200_num, dev_first_num, url_id, cache_body, ))
            results = []
...
def recycle_redis_task_dev_cache(url_id):
    ...
    try:
        ...
        logger.debug("recycle_redis_task_dev_cache url_id: %s|| dev_total_num: %s|| preload_200_num: %s" % (url_id, dev_total_num, preload_200_num))
        ...
        if update_body is None:
            ...
            return
        # logger.debug("recycle_redis_task_dev_cache url_id: %s|| type(update_body): %s|| update_body: %s" % (url_id, type(update_body), update_body))
        ...
        try:
            ...
            logger.error("recycle_redis_task_dev_cache [preload_result(%s) updating error]: %s" % (url_id, traceback.format_exc(), ))
        ...
        if preload_200_num and dev_total_num == preload_200_num:
            try:
                ...
                logger.debug("recycle_redis_task_dev_cache[insert error] url_id: %s|| error: %s " % (url_id, traceback.format_exc(), ))
        ...
        logger.error("recycle_redis_task_dev_cache[error] error: %s" % traceback.format_exc())
...
def preload_cancel(username, task_list):
    try:
        ...
        logger.debug("preload_cancel error: %s" % traceback.format_exc())
    ...
    try:
        ...
        for pre_url in pre_urls:
            ...
            if pre_url.get('status') == 'FAILED':
                ...
                old_result["devices"] = create_proload_result_dict(old_result.get('devices').values())
        ...
        logger.debug("execute_retry_task error: %s" % traceback.format_exc())
...
def save_fc_report(report_body):
    ...
    try:
        ...
        if not cache_body or not dev_cache:
            logger.debug("save_fc_report [%s] no cache_body %s or no dev_cache %s" %(url_id, cache_body, dev_cache))
            return
...
def get_unprocess(cache_body, url_id):
    ...
    try:
        ...
        logger.info('get_unprocess[all steps done.] url_id: %s|| unprocess: %s|| un_firstlayer_count: %s|| unprocess_un_firstlayer_count: %s' % (url_id, cache_body['unprocess'], un_firstlayer_count, unprocess_un_firstlayer_count))
...
def set_finished(url_id, cache_body, status):
    ...
    try:
        ...
        # logger.debug("set_finished insert error: %s|| url_id: %s " % (traceback.format_exc(e), url_id))
        pass
...
def dispatch_other(urls):
    try:
        ...
        pre_devs = [dev for dev in s1_db.preload_dev.find_one({'_id': urls[0].get('dev_id')}).get(
        ...
        results = send(pre_devs, urls)
        return {"results": results, "devices": pre_devs}
    except Exception:
        logger.debug("dispatch_other error: %s" % traceback.format_exc())
...
def init_pre_devs(channel_code, all_dev=False):
    ...
    #pre_devs.append({u'status': u'OPEN', 'code': 0, u'name': u'BGP-BJ-C-5H9', u'host': u'223.202.52.83', u'firstLayer': False, 'type': 'preload'})
        if channel_code !=0:
            ...
            elif int(device_type)==3 or int(device_type)==500:
        ...
        # #---for pddlx
        # if only_nf:
        #     pre_devs = [i for i in pre_devs if not i.get('firstLayer')]
...
def create_preload_result_dict_to_cache(devs):
    try:
        ...
        logger.debug("create_dev_dict_to_cache error: %s" % traceback.format_exc())
...
def worker(urls, pre_devs):
    ...
    try:
        ...
        if first_layer_first and f_and_nf:
            ...
            if f_res:
                results += f_res
        # logger.info("worker urls: %s|| results: %s" % (urls, results, ))
...
def send(devs, urls):
    ...
    try:
        ...
        # logger.debug("send dev_map: %s" % (dev_map, ))
        # original code
        ...
        if dev_map:
            ...
            try:
                ...
                retry_results = postal.retry(dev_map.values(), urls, pre_results_faild_dic)
            except Exception:
    ...
    try:
        ...
        if channel.get('callback', {}).get('email'):
            ...
            try:
                ...
                logger.debug("preload_callback email task_id: %s" % (callback_body.get("task_id")))
                email['email_url'] = channel.get('callback').get('email')
                email['code'] = 200
            except Exception:
                logger.debug("preload_callback email task_id: %s|| error: %s" % (callback_body.get("task_id"), traceback.format_exc()))
                email['code'] = 0
            email['send_times'] = 1
        ...
        if channel.get('callback', {}).get('url'):
            ...
            url['send_times'] = 1
            logger.debug("preload_callback url_post task_id: %s|| status: %s|| dest_url: %s" % (callback_body.get("task_id"), status, channel.get('callback', {}).get('url')))
            if status != 200:
                for i in range(3):
                    logger.debug("preload_callback url_post[retry: %s times.] task_id: %s|| status: %s" % (i, callback_body.get("task_id"), status))
                    status = doPost(channel.get('callback').get('url'), json.dumps(callback_body))
        ...
        if info and channel.get('username', '') == 'snda':
            ...
            logger.debug(params)
            for i in range(3):
                response = urllib.urlopen("http://116.211.20.45/api/preload_report?cdn_id=1005", params)
        ...
        try:
            ...
            logger.debug('preload_callback insert email_url_result error: %s' % traceback.format_exc())
    except Exception:
        logger.debug("preload_callback url_id: %s|| error: %s" % (callback_body.get("url_id"), traceback.format_exc()))
...
def judge_first_layer(id, host):
    ...
    try:
        ...
        logger.debug('judge_first_layer find _id: %s|| error: %s' % (id, traceback.format_exc()))
        return False
    try:
        if result:
...
def set_finished_by_api(url_id, date_s, status="FINISHED"):
    ...
    try:
        ...
        logger.debug('set_finished_by_api error: %s'% (traceback.format_exc()))
...
def get_all_channels(username):
    ...
    try:
        ...
        channel_list = json.loads(urllib2.urlopen(url, timeout=60).read())
