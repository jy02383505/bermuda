    ...
    for dev in devs:
        ...
        if first_layer_preload_len:
            if dev.get('firstLayer'):
                clients.append(HttpClient(dev.get('host'), port,
                                get_command(urls, urls[0].get("action"), dev.get('host'), conn_num=urls[0].get('conn_num'),
                                speed=urls[0].get('single_limit_speed'), test=test),
                                len(devs) * 200, my_map, connect_timeout, response_timeout))
            else:
                clients.append(HttpClient(dev.get('host'), port,
                                get_command(urls, urls[0].get("action"), dev.get('host'),
                                test=test),
                                len(devs) * 200, my_map, connect_timeout, response_timeout))
        else:
            clients.append(HttpClient(dev.get('host'), port,
