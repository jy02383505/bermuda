...
def high_work(urls):
    ...
    try:
        ...
        logger.debug("high 刷新使用的层次 layer_type: %s" % urls[0].get('layer_type'))
        tt1 = time.time()
        robot.dispatch(urls[0].get('layer_type'), urls)
        tt2 = time.time()
        logger.debug("high refreshRobot over t %s  url0_id is %s  urls len is %s" % (tt2 - tt1, urls[0].get('id'), len(urls)))

    except Exception, e:
        logger.warning('url_robot work error! do retry. error:%s' % traceback.format_exc(e))
...
def work(urls):
    ...
    try:
        ...
        tt2 = time.time()
        logger.debug(
            "refreshRobot over t %s  url0_id is %s  urls len is %s" % (tt2 - tt1, urls[0].get('id'), len(urls)))
...
class RefreshRobotURL():
    ...
    def load_basic_info(self, file_path):
        ...
        for line in fp:
            ...
            basic_info[use_name.upper()] = [tmp_list[1].strip(), tmp_list[2].strip(), tmp_list[3].strip(),
                                            tmp_list[4].strip(), tmp_list[5].strip()]
        return basic_info
    ...
    def dispatch(self, layer_type, urls):
        try:
            if not urls[0].get('devices'):
                ...
                # except Exception,e:
                #     logger.info('insert_refresh_result error urls:%s, error:%s' % (urls, traceback.format_exc(e)))
            ...
            try:
                max_level = max([layerNum.get('layerNum') for layerNum in self.db_dev.get('devices').values()])
            except Exception, e:
                logger.debug("url dispatch get max level error:%s, work reqeust id:%s" % (traceback.format_exc(e),
            ...
            while max_level >= 0:
                try:
                    ...
                    logger.debug('refresh dispatch work request id:%s  layer:%s, is_first_layer:%s,error:%s' %
                                 (self.get_id_in_requestofwork(), max_level, is_first_layer, traceback.format_exc(e)))
                max_level -= 1
            ...
            #                  (self.get_id_in_requestofwork(), traceback.format_exc(e)))
            ...
            logger.debug("url dispatch error=%s" % traceback.format_exc(e))
    ...
    def init_db_device(self, urls):
        ...
        try:
            devs += rcmsapi.getGrayDevices(urls[0].get("channel_code"))
        except Exception, e:
        ...
        for url in urls:
            ...
            db_update(db.url, {"_id": ObjectId(url.get("id"))}, {
                "$set": {"dev_id": self.dev_id, "worker_host": worker_hostname,
                         "recev_host": url.get("recev_host", "")}})
        logger.debug(
            "url_init_db_device successed ,worker_id: %s ,dev_id: %s " % (self.get_id_in_requestofwork(), self.dev_id))
        return db_device
    ...
    def layer_refresh(self, urls, layer_level):
        ...
        devs = [dev for dev in self.db_dev.get("devices").values() if dev.get('layerNum') == layer_level]
        ...
        try:
            for url_dict in urls:
                ...
                if url_dict['type'] == 'REFRESH_PRELOAD':
                    ...
                    db.refresh_preload.update({'url': url_dict['url'], "r_id": url_dict['r_id']},
                                              {'$set': {'callback': 'success', 'refresh_end_time': refresh_end}},
                                              multi=True)
        except Exception, e:
            logger.debug(e.message)
        logger.debug('save_device_results url update over by %s s ,dev_id %s, len url is %s' % (
    ...
    def insert_refresh_result(self, urls, devices):
        ...
        try:
            ...
            logger.info('insert_refresh_result error:%s' % traceback.format_exc(e))
            return
