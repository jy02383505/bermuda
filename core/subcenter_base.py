...
class SubBase(object):
    ...
    def init_mongo(self):
        ...
        try:
            ...
            for k, v in self.relation_sub_failed_dev.items():
            ...
            logger.debug("link_detection_refresh error:%s" % traceback.format_exc(e))
    ...
    def generate_orgin_ids(self):
        ...
        try:
            ...
            logger.debug("get task id error:%s" % traceback.format_exc(e))
    ...
    def generate_body(self):
        ...
        try:
            result = {}
            for dev, subcent_list in self.relation_sub_failed_dev.items():
            ...
            logger.debug("generate_body error:%s" % traceback.format_exc(e))
            return {}
    ...
    def send_result(self):
        ...
        for dev in self.body:
            ...
            try:
                ...
                if body_send:
                    clients.append(
                        HttpClient(subcenter_host, subcenter_port, body_send, len(body_send), my_map,
                                   self.connect_timeout, self.response_timeout))
            except Exception, ex:
                logger.debug("send content error :%s" % traceback.format_exc(ex))

        asyncore.loop(timeout=0.1, use_poll=True, map=my_map)
        for c in clients:
            dict_t = {}
            dict_t['host'] = c.host
            dict_t['code'] = c.response_code
            ...
            for dev, subcent_list in self.relation_sub_failed_dev.items():
                for subcent in subcent_list:
                    if subcent not in result:
                        ...
                        result[subcent].append(dev)
            for sub_center_host,failed_list in result.items():
	...
	:return:{ 'True'or 'False'}
        ...
        try:
            ...
            logger.debug("subcenter_update_result error:%s" % traceback.format_exc(e))
    ...
    def subcenter_udate_redis(cls, subcenter_result_mq, param, db, sub_center_host, edge_host, edge_code,
        ...
        try:
            ...
            logger.debug("subcenter_update_redis error:%s, uid:%s, sub_center_host:%s, edge_host:%s" %
                         (traceback.format_exc(e), param, sub_center_host, edge_host))
...
class Subcenter_Refresh_Url(SubBase):
    ...
    def insert_mongo(self):
        ...
        try:
            ...
            for sub_center_host_te,failed_list in result.items():
    ...
    def generate_command(self):
        ...
        try:
            ...
            logger.debug('subcenter_refresh_url generate_command error:%s' % traceback.format_exc(e))
            return None
    def generate_body(self):
        """
        generate the body
...
class Subcenter_cert(SubBase):
    ...
    def generate_command(self):
        ...
        try:
            ...
            logger.debug('subcenter_refresh generate_command error:%s' % traceback.format_exc(e))
            return None
        #try:
        #    return get_command(self.tasks)
        #except Exception, e:
...
class Subcenter_preload(SubBase):
    ...
    def generate_command(self):
        try:
            ...
            logger.debug('Subcenter_preload[generate_command] error: %s' % traceback.format_exc(e))
            return None
