...
def high_work(url):
    try:
        ...
        logger.warning('dir_robot work error! do retry. error:%s' % traceback.format_exc(e))
...
class RefreshRobotDIR():
    ...
    def dispatch(self, layer_type, url):
        try:
            ...
            try:
                ...
                logger.debug("dir  dispatch get max level error:%s, work reqeust id:%s" % (traceback.format_exc(e),
            ...
            logger.debug("dir dispatch error=%s" % traceback.format_exc(e))
        finally:
    ...
    def init_db_device(self, url):
        ...
        db_update(db.url, {"_id": ObjectId(url.get("id"))}, {
            "$set": {"dev_id": self.dev_id, "worker_host": worker_hostname, "recev_host": url.get("recev_host", "")}})
