        ...
        if urls[0].get('status') == 'TIMER':
            ...
            for url in urls:
                ...
                try:
                    ...
                    logger.debug("preload_worker update_one  error:%s, _id:%s" % (traceback.format_exc(e), url['_id']))
...
def save_fc_report(report_body):
    ...
    if has_lock:
        ...
        try:
            ...
            logger.debug("save_fc_report error:%s " % ex)
...
def reset_cache(cache_body, report_body, is_failed=False):
    ...
    :param is_failed:
    """
    for dev in cache_body.get('devices', {}).values():
...
def judge_first_layer(id, host):
    ...
    try:
        ...
        logger.deubug("judge_first_layer judge error:%s" % traceback.format_exc(e))
        return False
...
def init_pre_devs(channel_code):
    ...
    try:
        ...
        logger.debug("init_pre_devs error:%s " % traceback.format_exc(e))
...
def update_db_dev(dev_id, results):
    ...
    try:
        ...
        logger.debug("update_db_dev error:%s " % traceback.format_exc(e))
    ...
    try:
        ...
        if dev_map:
            ...
            try:
                ...
                retry_results = postal.retry(dev_map.values(), urls, "pre_ret", pre_results_faild_dic,
        ...
        logger.debug("send error:%s " % traceback.format_exc(e))
...
def get_bandwidth(first_layer_preload_len, speed):
    ...
    #     except Exception, e:
    #         logger.debug("division error:%s" % traceback.format_exc(e))
...
def parse_speed(speed):
    ...
    try:
        ...
        logger.debug('parse_speed error:%s' % traceback.format_exc(e))
        return 0
...
def save_fail_task(results, dev_id, urls, first_layer_preload_len=0, test=0):
    ...
    try:
        ...
        logger.debug("save_fail_task error:%s " % traceback.format_exc(e))
...
def reset_error_tasks(send_task, preload_task, host):
    for db_dev in s1_db.preload_dev.find({"_id": {"$in": [task.get("dev_id") for task in send_task]}}):
        db_dev["finish_time"] = datetime.now()
        for device in db_dev.get("devices").values():
    ...
    try:
        ...
        if channel.get('callback', {}).get('email'):
            ...
            try:
                ...
                logger.debug("preload_callback email error: %s url_id: %s " % (ex, callback_body.get("url_id")))
                email['code'] = 0
            email['send_times'] = 1
