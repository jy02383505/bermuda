...
def verify(username, password, remote_addr):
    ...
    if not ticket:
        try:
            user = json.loads(
                urllib.urlopen(CHECKIN_URL % urllib.urlencode({"username": username, "pwd": password, "ignoreCase": False})).read())
        except Exception,ex:
            logger.error(ex)
            logger.debug(CHECKIN_URL % urllib.urlencode({"username": username, "pwd": password}))
            logger.debug(urllib.urlopen(CHECKIN_URL % urllib.urlencode({"username": username, "pwd": password})).read())
