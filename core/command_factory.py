...
def get_command_json(urls, action, dev, check_conn=False):
    ...
    try:
        ...
        logger.debug('get_command_json except: %s' % traceback.format_exc(e))
...
def get_command(urls, action, dev_ip, conn_num=0, speed=0, test=0):
    ...
    try:
        ...
        logger.debug('get_command except: %s' % traceback.format_exc(e))



def re_handle_command(command):
    import preload_worker
    try:
        ...
        logger.debug('re_handle_command except: %s' % traceback.format_exc(e))
