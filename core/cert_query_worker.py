...
def dispatch(tasks):
    try:
        ...
        logger.debug("query cert trans error:%s " % traceback.format_exc(e))
...
def init_cert_dev(task):
    ...
    dev_id = s1_db.cert_query_dev.insert({'query_dev_ip':devs_dict, 'created_time':datetime.datetime.now(), 'unprocess': len(all_devs)})
...
def worker(tasks,send_devs):
    ...
    try:
        ...
        #make_all_callback_force([i['_id'] for i in tasks])
        logger.debug('cert_query trans worker error is %s' %(traceback.format_exc(e)))
...
def check_cert_query_task(task_ids):
    ...
    task_info_list = s1_db.cert_query_tasks.find({'_id': {'$in':[ObjectId(task_id) for task_id in task_ids]}, 'status':'PROGRESS'})
...
def update_db_dev(dev_id, results):
    ...
    try:
        ...
        s1_db.cert_query_tasks.update_many({'dev_id':ObjectId(dev_id)}, {'$set':{'finish_time': now}})

    except Exception, e:
        logger.debug('cert_query update_db_dev error is %s' %(traceback.format_exc(e))) 
    ...
    try:
        ...
        if dev_map:
            ...
            try:
                ...
                retry_results  = cert_query_postal.retry(dev_map.values(), tasks, results_faild_dic, Interface)
            except Exception,ex:
                retry_results = [] 
...
def set_finished(task_id, status):
    ...
    task_info = s1_db.cert_query_tasks.find_one_and_update({'_id':ObjectId(task_id),'status':"PROGRESS"},{"$set":{'status':status,'hpc_finish_time':now}}, return_document=ReturnDocument.AFTER)
    #cache -> mongo
    res_cache_key = '%s_res'%(task_id)
    dev_num_cache_key = '%s_res_dev_num'%(task_id)
    failed_cache_key = '%s_res_failed'%(task_id)
...
def make_op_log(username, remote_ip, cert_ids, _type):
    ...
    生成操作日志
    '''
    s1_db.query_op_log.insert({"username":username, "remote_ip":remote_ip, "task_ids":cert_ids, "type": _type, "created_time": datetime.datetime.now()})
