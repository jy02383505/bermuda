...
def process_loop_ret(http_results, dev_map):
    ...
    for result in http_results:
        try:
            ...
            logger.debug("cert_trans_postal process_loop_ret result has problem:%s, error:%s" % (result, traceback.format_exc(e)))
            try:
...
def get_command(tasks):
    ...
    for t in tasks:
        co = {'cert': t.get('cert',''), 'p_key': t.get('p_key', ''), 's_name': t.get('s_name'),'task_id': t.get('_id',''), 'seed': t.get('seed', ''), 'op_type': t.get('op_type', ''), 's_dir': t.get('s_dir', '')}
        commands.append(co)
