...
import logging
LOG_FILENAME = '/Application/bermuda/logs/refresh_last_hpcc_email.log'
            ...
            if r_id:
                ...
                if not email_to:
                    ...
                    try:
                        ...
                        logger.debug("refresh_failed_eamild error:%s" % traceback.format_exc(e))
            else:
                logger.debug('sleep 5 seconds')
                time.sleep(5)
    except Exception, e:
        logger.debug('autodesk_failed_eamild main error:%s' % traceback.format_exc(e))
    os._exit(0)
