...
import logging
logging.basicConfig(format = '%(asctime)s %(levelname)s - %(message)s', filename = '/Application/bermuda/logs/mq_agent.log')

def getServerName():
    try:
        server_name = re.search('\d+\.\d+\.\d+\.\d+',urllib2.urlopen('http://www.whereismyip.com').read()).group(0)
    except:
        try :
