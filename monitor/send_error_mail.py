...
sys.path.append('/Application/bermuda/')
import simplejson as json


import logging
logging.basicConfig(format='%(asctime)s - %(name)s - %(levelname)s - %(process)d - Line:%(lineno)d - %(message)s',filename='/Application/bermuda/logs/error_mail.log')
...
def send(db,tasks):
    ...
    try:
        for channel_code,tasks in merge(tasks).iteritems():
