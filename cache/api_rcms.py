...
GRAY_DEVICES_URL = config.get('rcmsapi', 'gray_devices_url')
...
class ApiRCMS(ApiOuter):
    ...
    def read_userInfo_rcms(self,username,times=1):
        return self.read_from_outer(CUSTOMER_URL % urllib.quote(username))

    def read_channels_rcms(self,username,times=1,need_retry=True):
        user_chnl_str = self.read_from_outer(CHANNELS_URL % urllib.quote(username),need_retry=need_retry)
        ...
        if self.get_valid_jsonstr(user_chnl_str):
            ...
            # 拒绝这两种状态的频道刷新功能。
            return [user_channel for user_channel in channel_rcms if user_channel['channelState'] not in ["TRANSFER", "CUSPAUSE", "SYSPAUSE"]]

        return []
    def read_devices_rcms(self,channel_code,times=1,need_retry=True):
        return self.read_from_outer(DEVICES_URL % urllib.quote(channel_code),need_retry=need_retry)

    def read_firstDevices_rcms(self,channel_code,times=1,need_retry=True):
        return self.read_from_outer(DEVICES_FIRSTLAYER_URL % urllib.quote(channel_code),need_retry=need_retry)

    def read_garyDevices_rcms(self,channel_code,times=1,need_retry=True):
        return self.read_post_cms_outer(GRAY_DEVICES_URL, channel_code, need_retry=need_retry)
