...
#         logger.error('redis api read_customer failed.username: %s,%s' % (username,traceback.format_exc(e)))
#
#     return customer
...
#     except Exception,e:
#         logger.error(traceback.format_exc(e))
#         logger.error('redis api read_customer_portal failed.username: %s,%s' % (username,traceback.format_exc(e)))
#
#     return customer
...
def read_channels(username):
    ...
    try:
        ...
        logger.error('redis api read_channels failed.username== %s ;exception==%s' % (username,traceback.format_exc(e)))
        logger.error(traceback.format_exc(e))
    return channels
...
def read_grayDevices(channel_code):
    logger.debug('user: request in read_grayDevices from channel code = %s' % channel_code)
    devices=[]
    devices_redis = rediscache.read_from_redis(rediscache.GARY_DEVICES_PREFIX , channel_code)
    try:
        if not devices_redis:
            ...
            device_maps = OutSysToRedis().render_gary_devices_rcms(channel_code)
            devices_redis = rediscache.read_from_redis(rediscache.GARY_DEVICES_PREFIX , channel_code)
        else:
...
def isValidChannel(channel_name,username):
    ...
    try:
        rediscache.append_to_channel_redis(rediscache.CHANNEL_LIST_KEY, channel_obj.get('code',0))
    except Exception,e:
        pass
