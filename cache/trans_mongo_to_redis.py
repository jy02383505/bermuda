...
class MongoToRedis(object):
    ...
    def render_channels_mongo(self,username):
        try:
            ...
            logger.error('render channel of user= %s from mongo to redis exception: %s' % (username,traceback.format_exc(e)))
    ...
    def render_devices_mongo(self,channel_code,isNotfirst=True):
        try:
            ...
            logger.error('render isnotfirst= (%s) device from mongo to redis exception: %s' % (str(isNotfirst),traceback.format_exc(e)))
    ...
    def render_portalUserInfo_mongo(self,username):
        try:
            ...
            logger.error('render portalUser Info_mongo of user= %s from mongo to redis exception: %s' % (username,traceback.format_exc(e)))
    ...
    #     except Exception,e:
    #         logger.error('refresh_redisCache exception:%s ' % traceback.format_exc(e))
    #         raise e
