...
class OutSysToRedis(object):
    ...
    def render_devices_rcms(self,channel_code,isNotFirst=True):
        ...
        return devices_map
    def render_gary_devices_rcms(self,channel_code):
        devices_map={}

        device_list = self.rcms_cache.read_garyDevices_rcms(channel_code,need_retry=False)
        #device_list =read_garyDevices_rcms(channel_code)
        #if device_list:
        devices_map= rediscache.refresh_gray_devices(channel_code,device_list)
        return devices_map