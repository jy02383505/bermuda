...
class ApiMongo(ApiRCMS,ApiPortal):
    ...
    def get_predays_users(self,days=5,now_str='',refresh_time=False):
        username_set=[]
    ...
    def get_all_rcms_users_new(self):
        ...
        try:
            ...
            logger.warn("get_all_rcms_channels error:%s" % traceback.format_exc(e))
            return 0, [], {}
        ...
        if not channels:
            ...
            try:
                for channel in channels:
                    ...
                    if state not in ["TRANSFER", "CUSPAUSE", "SYSPAUSE"]:
                        temp = {}
                        ...
                        if temp['customerName'] in map_temp:
                            ...
                            map_temp[temp['customerName']].append(temp)
                if map_temp:
                    map_keys = map_temp.keys()
                    return len(map_keys), map_keys, map_temp
                else:
                    return 0, [], {}
            except Exception, e:
                logger.debug("arrangement_all_channels error:%s" % traceback.format_exc(e))
                return 0, [], {}

    def sync_allObjects_rcms(self,all,refresh_time=False):
        # BATCH_SIZE=30
        # origin thread_num
        thread_num = 20
        # test thread num
        ...
        try:
            ...
            if all:
                ...
                usernum_countlatch, username_arr = self.get_predays_users(5, now_str,refresh_time=refresh_time)
            else:
            ...
            logger.error('exp detail is :%s' % traceback.format_exc(e))
            return False
    ...
    def sync_allObjects_portal(self):
        ...
        try:
            ...
            if all_users:
                ...
                for user_obj in all_users:
                    if user_obj.get('status') !='out':
                        ...
                        if (len(mapping.keys())==rediscache.PORTAL_USER_BATCH_SIZE):
            ...
            logger.error('From  portal to mongo sync allObjects_portal exception:%s' % traceback.format_exc(e))
            return False
    ...
    def sync_user_channel_portals_by_username(self, username):
        ...
        try:
            ...
            logger.error('sync user  channels for portal by username : [%s] to redis : %s' % (username,traceback.format_exc(e)))
    ...
    def sync_user_channel_portals_by_queue(self):
        ...
        try:
            ...
            for n in xrange(unread_num):
                username = self.read_channels_portal_changed_by_queue(getOne=True)
                self.sync_user_channel_portals_by_username(username)
                time.sleep(0.1)

        except Exception, e:
            logger.error('From portal to redis sync user channel portals by queue exception:%s' % traceback.format_exc(e))
            return False
    ...
    def sync_channels(self,username):
        try:
            ...
            logger.error('From rcms sync_channels method,in: %s have exceptions: %s' % (username,traceback.format_exc(e)))
            return []
    ...
    def sync_devices(self,channel_code=''):
        try:
            ...
            logger.error('From rcms sync_devices method in: %s have errors:%s' % (str(channel_code),traceback.format_exc(e)))
            # raise e
    ...
    def sync_firstLayer_devices(self,channel_code=''):
        try:
            ...
            logger.error('From rcms sync_firstLayer_devices of channel: %s method have errors: %s' % (str(channel_code),traceback.format_exc(e)))
            # raise e
    ...
    def sync_user_channel_portals(self,user_obj,mapping):
        try:
            ...
            logger.error('sync users and channels for portal by user : [%s] to mongo : %s' % (user_name,traceback.format_exc(e)))
    ...
    def read_channels_mongo(self,username):
        try:
            ...
            logger.error('read_channels_mongo from handle exception: %s ' % traceback.format_exc(e) )
        return []
    ...
    def read_portalChannels_mongo(self,username,isuser=False):
        try:
            ...
            logger.error('read_portalChannels_mongo exceptions: %s:' % traceback.format_exc(e))
        return {}
