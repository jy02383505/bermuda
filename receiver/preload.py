...
@preload.route('/preload/validChannels', methods=['GET', 'POST'])
def validChannels():
    '''
    当提供一个频道时，查询其是否已开通预加载；当提供一个用户名时，返回所有已开通的频道
    '''
    try:
        data = json.loads(request.data)
        logger.debug('validChannels data: %s|| remote_addr: %s' % (data, request.remote_addr))
        username = data.get('username')
        channel = data.get('channel')
        if not username and not channel:
            return jsonify({"code": 0, "msg": "Either username or channel must have one."})

        if username and not channel:
            channel_dbh = db.preload_channel.find({'username': username})
            channels = [c.get('channel_name') for c in channel_dbh]
            [PRELOAD_DEVS.set(c, '2') for c in channels if not PRELOAD_DEVS.exists(c)]
            return jsonify({"code": 0, "msg": channels})
        elif channel and not username:
            return jsonify({"code": 0, "msg": PRELOAD_DEVS.exists(channel)})
        return jsonify({"code": 0, "msg": "(%s)Channel: %s - Exists: %s" % (username, channel, PRELOAD_DEVS.exists(channel), )})
    except Exception:
        logger.error('validChannels [error]: %s' % (traceback.format_exc(), ))
...
def receiver_preload_json():
    ...
    try:
        if not request.data:
            return jsonify({"code": 410, "msg": "no post data."})
        data = json.loads(request.data)
        ...
        logger.error('receiver_preload_json[error]: %s' % (traceback.format_exc(), ))
...
def preload_avoid():
    ...
    try:
        ...
        return_dict.update({"code": 405, "msg": e.__str__()})
    except OverloadError as e:
        return_dict.update({"code": 406, "msg": e.__str__()})
    except InputError as e:
...
def receiver_preload_device():
    ...
    try:
        ...
        #---timer, interval, schedule task judgement
        ...
        return jsonify({"code": 407, "msg": e.__str__()})
    except InputError as e:
        return jsonify({"code": e.__code__(), "msg": e.__str__()})
    except NoChannelError as e:
        return jsonify({"code": 408, "msg": e.__str__()})
    except HTTPException as e:
        raise e
    except Exception as e:
        logger.debug(e.__str__())
        logger.debug('exception error:%s' % traceback.format_exc(e))
        return jsonify({"code": 500, "msg": "The schema of request is error."})
...
def preload_with_devices():
    ...
    try:
        ...
        if not tasks:
            return jsonify({"code": 404, "msg": 'tasks is empty.'})
        ...
        logger.error('preload_with_devices[error]: %s' % (traceback.format_exc(), ))
        return jsonify({"code": 500, "msg": "The schema of request is error."})
...
def receiver_preload_json_bak(request, queue_name='preload_task', return_dict=None, st=0, iname='/internal/preload/high'):
    ...
    if username in ['qq']:
        ...
        p_task, r_id = get_preload(r, type=data.get('type', 'API'))

    else:
        p_task, r_id = get_preload(request, type=data.get('type', 'API'))
    ...
    if not return_dict:
        return {"code": 0, "msg": "ok", 'r_id': r_id}
    else:
        return_dict.update({"code": 0, "msg": "ok", 'r_id': r_id})
        return return_dict
...
def get_wlk_devices(username, sign, domains=[]):
    ...
    *domains -> channels list.
    ...
    try:
        ...
        res = urllib.urlopen(big_preload_url, data=params)
...
def get_tasks_from_api(ticket, tasks_list, status_list, useUrlId=False):
    """
    {"username": "verycd",
    ...
    try:
        ...
        logger.debug("get_tasks_from_api condition: %s" % condition)
...
@preload.route('/preload/r_search', methods=['GET', 'POST'])
def queryTaskByRId():
    try:
        data = json.loads(request.data)
        ...
        username = request.args.get('cloud_client_name') if request.args.get('cloud_client_name') else data.get("username", "")
        if not r_id:
            logger.error('queryTaskByRId[no request_id.] data: %s' % (data, ))
            return jsonify({"code": 410, "msg": "no request_id."})

        request.url_id = [t['_id'] for t in s1_db.preload_url.find({'r_id': r_id})]
        return query_task_api()
    except Exception:
        logger.error('queryTaskByRId[error]: %s' % (traceback.format_exc(), ))
        return jsonify({"code": 500, "msg": "Request to queryTaskByRId() Transferred Fail."})
...
def query_task_api():
    ...
    try:
        ...
        if openapi_flag not in ['openapi.chinacache.com', 'openapi-test.chinacache.com']:
            ticket = authentication.verify(username, data.get("password", ""), request.remote_addr)
            count, tasks = get_tasks_from_api(ticket, tasks_list, status_list)
        else:
            # if data.get('url_id'):
            if hasattr(request, 'url_id') and request.url_id:
                count, tasks = get_tasks_from_api({'parent': username}, request.url_id, status_list, useUrlId=True)
            else:
...
def query_task_api_w():
    ...
    """
    try:
        logger.debug('query_task_api_w data:%s remote_addr:%s' %
...
def preload_report():
    ...
    st = time.time()
    try:
        logger.debug('preload_report remote_addr: %s|| data: %s' % (request.remote_addr, request.data, ))
        ...
        logger.debug('preload_report [Taking seconds: %s] report_body: %s' % ((time.time()-st), report_body))
        if send_type == 'xml':
            preload_worker.save_fc_report.delay(report_body)
        else:
            logger.debug('preload_report[/content/preload_report -- Taking seconds before save_fc_report.delay]: %s|| time.time(): %s' % (time.time()-st, time.time()))
            r_saveFcReport = preload_worker_new.save_fc_report.delay(report_body)
            logger.debug('preload_report[/content/preload_report -- Taking seconds after save_fc_report.delay]: %s|| r_saveFcReport: %s|| time.time(): %s' % (time.time()-st, r_saveFcReport, time.time()))
...
def doubleHttps(u):
    ...
    if u.startswith('https'):
        theOtherUrl = 'http%s' % u[5:]
    else:
        theOtherUrl = 'https%s' % u[4:]
    return theOtherUrl
...
def get_preload(request, ticket={}, type='other'):
    ...
    if isinstance(compressed, basestring):
        compressed = True if compressed.upper() == 'TRUE' else False
    task_list = []
    r_id = str(ObjectId())
    logger.debug('get_preload r_id: %s|| message: %s' % (r_id, message, ))
    ...
    if urls_cc:
        ...
        for task in message.get('tasks'):
            ...
            productType = getProductType(channel_code)
            task = {
                ...
                'compressed': True if productType == '1' else compressed, 'remote_addr': request.remote_ip if hasattr(request, 'remote_ip') else request.remote_addr,
                'recev_host': RECEIVER_HOST, 'is_avoid': message.get('is_avoid'), 'interval': message.get('interval'),
                'end_time': message.get('endTime'), 'task_type': message.get('taskType'), 'r_id': r_id}
            #---timer, interval, schedule tasks
            if task.get('task_type') and task.get('status') == 'PROGRESS':
                task['status'] = task.get('task_type')
            ...
            #---pddlx
            if task.get('username') in ['pddlx']:
                task_copy = deepcopy(task)
                task_copy['url'] = doubleHttps(task_copy.get('url'))
                task_copy['only_nf'] = True
                task_copy['status'] = 'TIMER'
                task_copy['task_type'] = 'TIMER'
                task_copy['start_time'] = datetime.strftime((datetime.now() + timedelta(seconds=15)), '%Y-%m-%d %H:%M:%S')
                task_list.append(task_copy)


    logger.debug('get_preload r_id: %s|| task_list: %s' % (r_id, task_list, ))
    return task_list, r_id
    ...
    try:
        ...
        for task in tasks:
            ...
            task = {
                ...
                'compressed': True if productType == '1' else compressed,
            ...
            if task.get('task_type') and task.get('status') == 'PROGRESS':
                task['status'] = task.get('task_type') # TIMER, INTERVAL, SCHEDULE
            task_list.append(task)
        return task_list
    except Exception:
        ...
        for task in message.get('tasks'):
            ...
            if task.get('url', '')[0] == '*':
                ...
                if all_username_channel:
                    for key, value in all_username_channel.items():
                    ...
                    url_search = "https://portal-api.chinacache.com:444/api/internal/getBusinessInfo.do?allCustomerBusinesses=true&username=" + \
                        str(username)
                    user = json.loads(urllib.urlopen(url_search).read())
...
def get_fc_report(request_data):
    ...
    try:
        ...
        try:
            ...
            logger.error("http_status_code [error]: %s" % traceback.format_exc())
        ...
        logger.debug("get_fc_report error: %s" % traceback.format_exc())
        raise InputError(500, "The schema of request is error.")
...
def get_billing(channel_code, query_day):
    ...
    ret = {}
    for i in range(24):
        begin = query_day + relativedelta(hours=i)
...
def query_task_forward_from_webluker(username, tasks_list, status_list):
    ...
    try:
        req = urllib2.Request(url=requrl, data=test_data_urlencode, headers={'Content-type': 'application/json'})
...
def finishd_preload():
    ...
    try:
        ...
        logger.error('finishd_preload [error]: %s' % traceback.format_exc())
        return jsonify({"code": 500, "msg": "The schema of request is error."})
...
def had_preload_url(username, url):
    ...
    try:
        ...
        logger.error('had preload_url [error]: %s' % (traceback.format_exc()))
        return False
...
def get_avoid_channel(channel_code=''):
    try:
        ...
        logger.debug('get_avoid_channel[error]: %s' % (traceback.format_exc(e)))
        return False, 0


def isScheduledTask(data):
    taskType = data.get('taskType')
    interval = data.get('interval')
    startTime = data.get('startTime')
    endTime = data.get('endTime')
    if taskType.upper() == 'TIMER':
        if not startTime:
            raise InputError(410, 'TIMER tasks should have field startTime.')
    elif taskType.upper() == 'INTERVAL':
        if (not interval) or (not endTime):
            raise InputError(410, 'INTERVAL tasks should have field interval and endTime.')
    elif taskType.upper() == 'SCHEDULE':
        if (not interval) or (not startTime) or (not endTime):
            raise InputError(410, 'SCHEDULE tasks should have field interval, startTime and endTime.')


def getProductType(channel_code):
    try:
        p_key = 'channel_code_%s' % (channel_code, )
        productType = CHECK_CACHE.get(p_key)
        if not productType:
            productType = db.preload_channel.find_one({'channel_code': channel_code}).get('productType')
            CHECK_CACHE.set(p_key, productType)
            CHECK_CACHE.expire(p_key, 60*5)
            logger.debug("isWebChannel [inIF:productTypeNotExist] productType: %s|| channel_code: %s" % (productType, channel_code, ))
        logger.debug("isWebChannel productType: %s|| channel_code: %s" % (productType, channel_code, ))
    except Exception:
        logger.error('getProductType channel_code: %s|| [error]: %s' % (channel_code, traceback.format_exc(), ))
    return productType
