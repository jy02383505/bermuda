    ...
    if adapters.is_verify_encrypt_search_failed(rid, request.args.get("v")):
        ...
        if not result:
            ...
            if refresh_request:
                ...
                raise NotFound(u'%s not found.' % rid)
        else:
...
def get_url_result(url):
    ...
    failDeviceCount = len(
        [dev for dev in db_dev.get("devices", []).values() if dev.get('code') in [503, 404, 408]]) if db_dev else 0
    if url.get("r_dev_id"):
...
def internal_search_dev(url_id):
    ...
    try:
        ...
        if db_url.get("r_dev_id"):
            ...
            for dev in db_dev.get("devices").values():
        ...
        logger.debug("internal_search_dev error:%s" % e)
        return jsonify({})
...
def internal_search_dev(url_id):
    ...
    try:
        ...
        result = {"failNodes": failed_dev.values()}
        return jsonify(result)
    except Exception, e:
        logger.debug("internal_search_dev error:%s" % e)
        return jsonify({})
...
def content_query_failed():
    ...
    try:
        ...
        logger.info('content query_failed get start_time  end_time error:%s' % traceback.format_exc(e))
    ...
    try:
        ...
        logger.debug('content query_failed error:%s' % traceback.format_exc(e))
    ...
    try:
        ...
        logger.debug('query failed_task error:%s' % traceback.format_exc(e))
...
def query_all_refresh_content():
    ...
    try:
        ...
        logger.debug('query_all_refresh_content  json loads error:%s, request.data:%s, remote_ip:%s'
                     % (traceback.format_exc(e), request.data, request.remote_addr))
    ...
    try:
        ...
        logger.debug('query_all_refresh_content parse data error:%s' % traceback.format_exc(e))
...
def get_node_info():
    ...
    try:
        ...
        logger.debug("query get_node_info  find mongo error:%s" % ex)
    if device_info:
