...
import adapters
import result_tencent
import preload
import query
import certificate_transport
...
from util_rec import get_device_failed_url, device_failed_url_to_txt, get_start_pull_time,change_pull_time
...
DEVICE_NAME_IP = redisfactory.getDB(9)
...
def create_app(db_session, query_db_session):
    ...
    def receive_Api():
        ...
        # add 2017-9-5
        update_urls_cc, update_urls_web = webluker_tools.get_urls(task['username'], task.get('update_urls'))
        purge_dirs_cc, purge_dirs_web = webluker_tools.get_urls(task['username'], task.get('purge_dirs'))
...
　　          500   　　服务器错误 　　需要联系客服人员
        ...
        # except Exception, e:
        #     logger.debug('splitter_new submit error:%s' % traceback.format_exc(e))
        #     user_list = []
        task_all = {}
        task_all['username'] = username
        task_all['urls'] = task.get('urls')
    ...
    def sub_center_result():
        ...
        if not ack_content:
            ...
            try:
                ...
                logger.debug("receiver receiveSubCenterResult  query retry_device_branch_list error:%s" % e)
            ...
            if retry_device_branch_list:
                ...
                for retry_device_branch in retry_device_branch_list:
                    ...
                    result = update_retry_branch(retry_branch_id, branch_code, host, sub_center_ip, rid, refresh_type)
            ...
            try:
                ...
                if not node_dic:
                    ...
                    flag = True
                    for node_id in node_dic.keys():
                        logger.debug("receiver sub_center_result node_id:%s, code:%s" % (node_id, node_dic[node_id]))
                        try:
                            ...
                            logger.debug("receiver receiveSubCenterResult retry_branch_content error:%s" % e)
                ...
                logger.debug("receiver  receiveSubCenterResult parse xml error:%s" % traceback.format_exc(e))
...
　　                  "email":["mail1","mail2"],
    ...
    def adapter_tencent_search():
        ...
        if request.args.get('begin_time') and request.args.get('end_time'):
            ...
            return response
        raise BadRequest(u"请求必须包含begin_time和end_time参数。")
    ...
    def adapter_snda():
        ...
        password = request.form.get("password", "")
        time = request.form.get("time", "")
        info = "snda request %s with {user:%s,remote_addr:%s}" % (request.path, username, request.remote_addr)
        logging.debug(info)
        # 验证成功 则返回portal校验密码
    ...
    def adapter_encrypt():
        params = json.loads(request.data)
        logger.debug('new encrypt request{user:%s,remote_addr:%s}' % (params.get("username"), request.remote_addr))
    ...
    def restart_agentd():
        ret_pid = commands.getstatusoutput("ps -eo pid,cmd|grep agentd|grep -v grep|awk '{print $1}'")
        if not ret_pid[0]:
            ret_kill = commands.getstatusoutput("kill -9 %s" % ret_pid[1])
            if not ret_kill[0]:
                ret_restart = os.system("nohup /Application/bermuda/bin/python /Application/bermuda/bin/agentd &")
        return commands.getstatusoutput("ps ux | grep agentd |grep -v grep | awk '{print $9}'")[1]
    ...
    def isValidUrlByPortal():
        ...
        logger.debug('isValidUrlByPortal  key:%s, username:%s, channel_name:%s' % (key, username, channel_name))
    ...
    @app.route('/fail/device/pull/<hostname>', methods=['GET'])
    def monitor_device_txt(hostname):
        '''
        生成txt文件
        '''
        # host_name = request.args.get('host')
        name_host = DEVICE_NAME_IP.hget('DEVICE_IP_NAME_IP', hostname)
        if name_host:
            hostname = name_host
        ...
        if start_time:
            js_date_l_s = start_time.split('/')
            start_date = '%s-%s-%s' % (js_date_l_s[2], js_date_l_s[0], js_date_l_s[1])
            start_d = datetime.datetime.strptime(start_date + ' ' + '00:00:00', '%Y-%m-%d %H:%M:%S')
        if end_time:
            js_date_t_s = end_time.split('/')
            end_date = '%s-%s-%s' % (js_date_t_s[2], js_date_t_s[0], js_date_t_s[1])
            end_d = datetime.datetime.strptime(end_date + ' ' + '00:00:00', '%Y-%m-%d %H:%M:%S')

        app.logger.debug("hostname%s,start_time:%s,end_time:%s" % (hostname,start_time, end_time))
        # end_time ,start_time ==> (now, pull_time) or (now,suspend_time) or (now ,10000num)
        #
        if not end_d:
            end_d = datetime.datetime.now()
        if not start_d:
            start_d = get_start_pull_time(hostname)
        if not start_d:
            txt_name = '%s_to_%s_%s' % ("actime", end_d.strftime('%Y-%m-%d_%H:%M:%S'), hostname)
        else:
            txt_name = '%s_to_%s_%s' % (start_d.strftime('%Y-%m-%d_%H:%M:%S'), end_d.strftime('%Y-%m-%d_%H:%M:%S'), hostname)

        res = get_device_failed_url(hostname, start_d, end_d)
        app.logger.debug("txt_name:%s" % txt_name)
        # app.logger.debug("res:%s" % res)
        device_failed_url_to_txt(txt_name, res)
        change_pull_time(hostname, end_d)

        # return redirect('/%s.txt.gz' % (txt_name))
        return send_from_directory('/Application/bermuda/static/log_txt/','%s.txt.gz' % (txt_name),as_attachment=True)
    ...
    def url_physical():
        ...
        try:
            ...
            logger.debug("{}".format(traceback.format_exc(e)))
    ...
    def get_url_details(urls):
        ...
        for url in urls:
            try:
                ...
                if domain and  code :
                    urls_list.append({"id":get_uuid(),"channel_code":code,"domain":domain,"url":url,"newphysical":"newphysical"})
                else:
                    logger.debug("physical error url is ".format(url))
            except Exception,e:
                logger.debug("get url details physical {}".format(traceback.format_exc(e)))
        return urls_list
    ...
    def rep_request_result():
        ...
        try:
            ...
            for url in urls_list:
                ...
                rep_res_dev = sorted(refresh_results, key=lambda re_r: re_r.get('time'), reverse=False)
                ...
                try:
                    ...
                    logger.debug('send email error {0}'.format(traceback.format_exc(e)))
            return json.dumps(all_result)
        except Exception, e:
            logger.debug('rep request error {0}'.format(traceback.format_exc(e)))
            return json.dumps({'code': 500, 'message': 'error'})

    @app.route("/internal/rep/request/result", methods=['GET', 'POST'])
    def internal_rep_request_result():
        ...
        try:
            ...
            for url in urls_list:
                # tmp_uper_dict = {}
                dev_dict = dev_ip_dict.get(url.get('dev_id'))
                fai_devices = fail_send_devices.get(url.get('dev_id'))
                if dev_dict:
                    tep_ip_dict = dev_dict
                else:
                    db_devs = get_devs_by_id(url.get('dev_id'))
                    devs = db_devs.get("devices").values()
                    try:
                        # tep_name_dict = {i.get("name").lower:i.get("name") for i in devs if i['code'] != 204 and i['type'] != 'FC'}
                        tep_ip_dict = {i.get("host"): i.get("name") for i in devs if
                                       i['code'] not in [204, 502, 503] and i['type'] != 'FC'}
                        fai_devices = [i.get("name") for i in devs if
                                       i['code'] in [502, 503] and i['type'] != 'FC']
                    except KeyError:
                        tep_ip_dict = {}
                        fai_devices = []
                    dev_ip_dict[url.get('dev_id')] = tep_ip_dict
                    fail_send_devices[url.get('dev_id')] = fai_devices
                # logger.debug("---------------------url--{0}".format(url))
                host_name_map = {host_name.lower(): host_name for host_name in tep_ip_dict.values()}
                tmp_request_list = [ip for ip in tep_ip_dict]
                refresh_results = get_refresh_result_by_sessinId(url.get('u_id'))
                refresh_results = copy.deepcopy(list(refresh_results))
                # print refresh_results
                # logger.debug("---refresh_results{0}".format(refresh_results))
                rep_devices_map = {x.get("host", ""): x.get("hostname", "") for x in refresh_results}
                rep_devices = [x.get("host", "") for x in refresh_results]
                fail_devices = [x.get("host", "") for x in refresh_results if x['result'] != "200"]

                set_all_devices_ip = set(tmp_request_list)
                set_rep_devices_ip = set(rep_devices)
                set_fail_devices = set(fail_devices)
                # print set_all_devices_ip
                # print set_rep_devices_ip
                not_result_devices = set_all_devices_ip - set_rep_devices_ip

                rep_over_devices_map = {rep_devices_map.get(ip).lower(): rep_devices_map.get(ip) for ip in
                                        (set_rep_devices_ip - set_all_devices_ip)}

                not_result_name_map = {tep_ip_dict.get(no_ip).lower(): tep_ip_dict.get(no_ip) for no_ip in
                                       not_result_devices}

                not_result_cha = set(not_result_name_map.keys()) - set(rep_over_devices_map.keys())

                not_result_name = [not_result_name_map.get(hostname) for hostname in not_result_cha]

                rep_over_cha = set(rep_over_devices_map.keys()) - set(not_result_name_map.keys())
                rep_over_devices = [rep_over_devices_map.get(hostname) for hostname in rep_over_cha]
                result_fail_devices = [tep_ip_dict.get(host, host) for host in set_fail_devices]
                result_fail_devices = result_fail_devices + fai_devices
                all_result.append({"session_id": str(url.get('u_id')), 'url': url.get('url'),
                                   'create_time': task_tt,
                                   "not_result_devices": "{}".format(not_result_name),
                                   "fail_devices": "{}".format(result_fail_devices),
                                   "rep_over_devices": "{}".format(rep_over_devices)
                                   })
            return json.dumps(all_result)
        except Exception, e:
            logger.debug('rep request error {0}'.format(traceback.format_exc(e)))
    ...
    def subcenter_preload():
        ...
        if not ack_content:
            ...
            # return handing_failed_sub_result(callback_params, sub_center_host, edge_host, branch_code)
        ...
        try:
            try:
                ...
                logger.debug('subcenter_preload error:%s' % traceback.format_exc(e))
                branch_code = 503
                for param in callback_params:
                    Subcenter_preload.subcenter_udate_redis('cert_subcenter_result', param, db_s1, sub_center_host,
    ...
    def sub_center_result_new():
        ...
        try:
            try:
                ...
                logger.debug('getCodeFromData error:%s' % traceback.format_exc(e))
                branch_code = 503
                for param in callback_params:
                    Subcenter_cert.subcenter_udate_redis('cert_subcenter_result', param, db_s1, sub_center_host,
                                                         edge_host, branch_code, date_time_now)
            ...
            logger.debug("receive SubCenterResult  error:%s" % traceback.format_exc(e))
            return json.dumps({'msg': 'error', 'content': ' error:%s' % e})
    ...
    def get_best_road():
        ...
        try:
                    ...
                    if keys and len(keys) > 0:
                        ...
                        for key in sub_min_list:
                            if len(best_road_sor_list) >= FIRST_HOST:
                                ...
                                logger.debug('get best road branch_center_list %s' % (branch_center_list))
            ...
            logger.debug('get_best_road_v error:%s, host:%s' % (traceback.format_exc(e), host))
    ...
    def sub_center_refresh_result():
        ...
        # create_time = db_s1['subcenter_refresh_task'].find_one({"r_id": request_id}).get("created_time")
        create_time = db_s1['subcenter_refresh_task'].find_one({"_id": ObjectId(task_id)}).get("created_time")
