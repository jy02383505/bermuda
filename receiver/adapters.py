...
def get_tencent_task(ori_task, remote_addr):
    try:
        ...
        if is_verify_request_timeout(ori_task.get('request_time')):
            return refresh_task, False, u'{"ret": -102, "msg": "请求超时"}'
        ...
        raise BadRequest(u"%s 格式不正确。" % ori_task)
    except Forbidden:
        return refresh_task, False, u'{"ret": -101, "msg": "未授权的上报IP:%s"}' % remote_addr
...
def is_verify_tencent_failed(ori_task):
    md5 = hashlib.md5()
    params_key = u"%s%s%s%s%s" % (ori_task.get('request_time'), str(ori_task.get('serial_num')), ''.join(ori_task.get('update_urls', '')) , ''.join(ori_task.get('delete_urls', '')) , CDN_KEY_TENCENT)
...
def is_verify_ntease_failed(ori_task, remote_addr):
    ...
    md5 = hashlib.md5()
    params_key = u"%s%s%s%s" % (time.strftime('%Y%m%d', time.localtime()), ori_task.get('username'), CDN_KEY_NTEASE , PASSWORD_KEY_NTEASE)
    md5.update(params_key.encode('utf-8'))
...
def is_verify_encrypt_receiver_failed(ori_task):
    md5 = hashlib.md5()
    params_key = u"%s%s%s%s%s" % (time.strftime('%Y%m%d', time.gmtime()), ori_task.get('username'),CDN_KEY_ENCRYPT,''.join(ori_task.get('urls')),''.join(ori_task.get('dirs')))
    md5.update(params_key.encode('utf-8'))
    return md5.hexdigest() != ori_task.get('verify')

def is_verify_encrypt_search_failed(rid,verify):
    md5 = hashlib.md5()
    params_key = u"%s%s%s" % (time.strftime('%Y%m%d', time.gmtime()), rid ,CDN_KEY_ENCRYPT)
    md5.update(params_key.encode('utf-8'))
