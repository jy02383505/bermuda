...
class thread_t_f(threading.Thread):
    ...
    def run(self):
        try:
            ...
            logger.debug("find url error:%s" % traceback.format_exc(e))
            result_all_failed = []
        if result_all_failed:
            ...
            for res in result_all_failed:
                ...
                devices = devices_t.get('devices').values()
                all = 0
                suspend = 0
                failed = 0
                success = 0
                ...
                try:
                    ...
                    logger.debug('failed_task insert error:%s' % traceback.format_exc(e))
            ...
            if len(list_insert) > 0:
                try:
                    ...
                    logger.debug('last insert list_insert failed_task error:%s' % traceback.format_exc(e))
...
def query_device_depend_id(dev_id):
    ...
    try:
        ...
        logger.debug('query_device_depend_id find device error:%s' % traceback.format_exc(e))
        return devices
    if devices_result:
        devices_devs = devices_result.get('devices')
        if devices_devs:
            for dev in devices_devs.values():
                data_temp = {}
...
def query_retry_device_depend_id(retry_branch_id):
    ...
    try:
        ...
        logger.debug('query_retry_device_depend_id find device error:%s' % traceback.format_exc(e))
        return devices
    if devices_result:
