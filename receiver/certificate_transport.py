# -*- coding:utf-8 -*-
import traceback, random, zipfile, zlib, socket,urllib2,hashlib
...
def cert_trans():
    ...
    try:
        ...
        return jsonify({'code': 200, 'task_id': task['_id'], 'cert_id': task['c_id'], 'isSafe': (lambda x : True if x ==False else False) (task.get('middle_cert_lack',False)),'cert_cache_name': task['s_name']})
    except CertInputError, ex:
        return jsonify({"code": 504, "msg": ex.__str__()})
    except CertExpireError, ex:
        return jsonify({"code": 505, "msg": ex.__str__()})
    except CertRevokeError, ex:
        return jsonify({"code": 506, "msg": ex.__str__()})
    except CertPrikeyError, ex:
        return jsonify({"code": 507, "msg": ex.__str__()})
    except CertPathError, ex:
        return jsonify({"code": 508, "msg": ex.__str__()})
    except CertDecryptError, ex:
        return jsonify({"code": 509, "msg": ex.__str__()})
    except CertPrikeyTypeError, ex:
        return jsonify({"code": 510, "msg": ex.__str__()})
    except CertSaveNameError, ex:
        return jsonify({"code": 511, "msg": ex.__str__()})
    except CertNoRoot, ex:
        return jsonify({"code": 512, "msg": ex.__str__()})
    except CertNoMiddle, ex:
        return jsonify({"code": 513, "msg": ex.__str__()})
    except CertAliasError, ex:
...
def transfer_expired_cert():
    ...
    try:
        ...
        try:
            ...
            logger.error('callback error %s' % (traceback.format_exc(e)))
            return jsonify({'code': 504, 'msg': 'delete error%s' % (traceback.format_exc(e))})
        ...
        for save_name in array_s_name:
            ...
            try:
                cert_trans_worker.remove_certName_redis(save_name) # add 20190415
            except Exception,e:
                logger.error('remove_certName error %s' % (traceback.format_exc(e)))
...
def transfer_portal_expired_cert():
    ...
    try:
        ...
        for save_name in array_s_name:
            ...
            s1_db.cert_detail.update({'save_name':save_name},{"$set" : {"t_id" : t_id,"cert_alias":change_name}})
            try:
                s1_db.cert_only_domain.update({'save_name':save_name},{"$set":{'isTrans': True}})#one domain cert 201812
            except Exception,e:
                logger.debug(traceback.format_exc(e))
            #if not info:
            #    raise CertNotFoundError()
            try:
                cert_trans_worker.remove_certName_redis(save_name)
            except Exception, e:
                # logger.debug(traceback.format_exc(e))
                logger.error('remove_certName error %s'%(traceback.format_exc(e)))
...
def cert_trans_dev_pull():
    ...
    try:
        ...
        cert_encrypt_name = get_only_cert()
        cert_decrypt_name = get_decrypt_cert() #加密的文件解密
        ...
        if ignore_n == None:
            ...
            if cert_version =="encrypt":
                for cert_name in cert_encrypt_name + cert_decrypt_name:
                    try:
                        ignore_n.remove(cert_name)
                    except Exception, e:
                        logger.debug(traceback.format_exc(e))
        ...
        if cert_version == "encrypt":
            for a_cert in all_certs:
                if a_cert.get("s_name") in cert_encrypt_name:
                    a_cert['is_encrypt'] ='true'
...
@certificate_transport.route("/internal/cert/get/all", methods=['GET', 'POST'])
def cert_get_all_now():
    '''
    设备同步所有证书
    '''
    dict_all = {}
    try:
        ...
        all_certs = cert_trans_worker.get_all_cert_work()
        all_certs = set(all_certs)
        logger.debug('cert get part cert remote_ip %s, all_certs len  is %s' % (request.remote_addr, len(all_certs)))
        cert_trans_worker.make_op_log(hostname, request.remote_addr, [], 'dev_part_pull')
        if cert_version == "encrypt":
            only_cert = set(cert_encrypt_name)
            dict_all['work_cert_list'] = list(all_certs-only_cert)
            dict_all['encrypt_list'] = list(only_cert)
        else:
            dict_all['work_cert_list'] = list(all_certs)
            dict_all['encrypt_list'] = []
        # return make_response(json.dumps(all_certs))
    except :
        pass
    return make_response(zlib.compress(json.dumps(dict_all)))


@certificate_transport.route("/internal/cert/get/list",methods=['GET', 'POST'])
def cert_get_list_cert():
    '''
    设备同步所有证书
    '''
    try:
        ...
        all_certs = cert_trans_worker.get_part_cert(cert_list)
        logger.debug('cert get part cert remote_ip %s, all_certs len  is %s' %(request.remote_addr, len(all_certs)))
        cert_trans_worker.make_op_log(hostname, request.remote_addr,[], 'dev_part_pull')
        if cert_version == "encrypt":
            for a_cert in all_certs:
                if a_cert.get("s_name") in cert_encrypt_name+cert_decrypt_name:
                    a_cert['is_encrypt'] ='true'
        #return make_response(json.dumps(all_certs))
        return make_response(zlib.compress(json.dumps(all_certs)))
...
@certificate_transport.route("/internal/cert/check",methods=['GET', 'POST'])
def cert_all_check():
    '''
    设备同步所有证书
    '''
    try:
        ...
        dev_cert = set(dev_cert)
        all_certs = cert_trans_worker.get_all_cert_work()
        all_certs = set(all_certs)
        lack_cert = all_certs - dev_cert
        code = 206
        status = "fail"
        if len(lack_cert) == 0:
            code = 200
            status = "success"
        return jsonify({"code": code, "status": status, "lack": ",".join(lack_cert)})
    except Exception, e:
        logger.debug('/internal/cert/apply/callback error remote_addr %s'%(request.remote_addr))
        logger.debug(traceback.format_exc(e))
        return jsonify({"code": 500, "status": "fail", "lack": "error"})
...
def get_only_cert():
    name_list = []
    all_encrypt = s1_db.cert_only_domain.find({"Is_encry" : True,"isTrans" : False})
    for only_cert in all_encrypt:
        name_list.append(only_cert.get("save_name"))
    return name_list

def get_decrypt_cert():
    name_list = []
    all_encrypt = s1_db.cert_only_domain.find({"Is_encry" : False,"Is_change":True,"isTrans" : False})
    for only_cert in all_encrypt:
        name_list.append(only_cert.get("save_name"))
    return name_list
...
def make_task(data, _type='portal'):
    ...
    '''
    if _type == 'portal':
        logger.debug('portal send cert data is :{}'.format(data))
        s1_db = database.s1_db_session()
        ...
        cert_alias = data.get('cert_name', 'chinacache-cert:{}'.format(time.time()))#20190529
        seed = cert_trans_worker.get_custom_seed(username)

        is_rigid = data.get('rigid',False)
        hasFc = data.get('has_fc', False)
        ...
        # 2019/03/26 anna.miao
        pri_type = ""
        ecc_type = False
        if 'BEGIN PRIVATE KEY' in p_key:
            pri_type = "pcks8"
        if "EC PRIVATE" in p_key:
            ecc_type = True
        ...
        #检查子证书和私钥match
        if ecc_type:
            if not cert_tools.check_ecc(all_cert_checked, p_key):
